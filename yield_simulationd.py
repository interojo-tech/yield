    ...
    if st:
        ...
        def compute_combined_summaries(full_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
            ...
            if not weekly_combined.empty:
                # Respect the selected year for weekly charts. If the sidebar is set to
                # '전체', fall back to the latest available year so that older yearly
                # data does not dominate the recent-week view.
                try:
                    target_year = int(selected_year) if str(selected_year) != '전체' else int(pd.to_numeric(weekly_combined['년도'], errors='coerce').max())
                    weekly_combined = weekly_combined[pd.to_numeric(weekly_combined['년도'], errors='coerce') == target_year].copy()
                except Exception:
                    pass
                ...
                if len(weekly_combined) > 10:
                    weekly_combined = weekly_combined.tail(10)
            else:
                # No weekly data
        ...
        with comb_cols[1]:
            ...
            draw_combined_chart(weekly_combined.tail(min(10, len(weekly_combined))) if not weekly_combined.empty else weekly_combined, "주간 양품량 및 수율", "주차", (65.0, 85.0))
        ...
        for proc in PROCESS_ORDER:
            ...
            with proc_cols[1]:
                draw_line_chart(proc_weekly_summary.tail(min(10, len(proc_weekly_summary))) if not proc_weekly_summary.empty else proc_weekly_summary, f"{proc} 주간 수율", "주차")
            y_min, y_max = _orig_ymin, _orig_ymax
