...
def _sort_year_week_labels(labels) -> list:
    def _key(v):
        s = str(v)
        m = re.search(r"(\d{4})-W(\d+)", s)
        if m:
        ...
        if len(nums) == 1:
            return (0, int(nums[0]))
        return (0, 0)
    return sorted(list(labels), key=_key)


def _limit_recent_weeks_df(summary: pd.DataFrame, n: int = 10) -> pd.DataFrame:
    if summary is None or summary.empty:
        return summary
    ordered = _sort_year_week_labels(summary.index)
    tmp = summary.loc[ordered]
    return tmp.tail(min(n, len(tmp)))
...
def main() -> None:
    ...
    """
    # Determine yield workbook paths. Quarterly files are preferred and merged automatically.
    base_dir = os.path.dirname(__file__)
    file_paths = discover_yield_files(base_dir)
    if not file_paths:
        msg = (
            "수율 엑셀 파일을 찾지 못했습니다. 같은 폴더에 분기 파일 "
            "'공정기술팀 대시보드(수율)_26.1q.xlsx' 형식 또는 기존 파일명을 업로드해 주세요."
        )
        if st:
    ...
    if st:
        ...
        def _weekly_label_sort_key(label) -> pd.Timestamp:
            """Convert a weekly label like 2026-W12 into a sortable timestamp."""
            text_label = str(label)
            match = re.search(r"(\d{4})-W(\d{1,2})", text_label)
            if match:
                ...
                try:
                    ...
                    return pd.Timestamp(year=year, month=1, day=1) + pd.to_timedelta((week - 1) * 7, unit='D')
            match = re.search(r"W(\d{1,2})", text_label)
            if match:
                week = int(match.group(1))
                base_year = int(selected_year) if str(selected_year).isdigit() else 1900
                week = max(1, min(week, 53))
                try:
                    return pd.Timestamp.fromisocalendar(base_year, week, 1)
                except Exception:
                    return pd.Timestamp(year=base_year, month=1, day=1) + pd.to_timedelta((week - 1) * 7, unit='D')
            return pd.Timestamp('1900-01-01')

        def _sort_limit_weekly_df(summary: pd.DataFrame, limit: int = 10) -> pd.DataFrame:
            """Sort weekly summaries chronologically and keep only the most recent rows."""
            if summary is None or summary.empty:
                return summary
            tmp = summary.copy()
            tmp['__label__'] = [str(idx) for idx in tmp.index]
            tmp['__sort_key__'] = tmp['__label__'].apply(_weekly_label_sort_key)
            tmp = tmp.sort_values('__sort_key__').tail(limit)
            tmp = tmp.set_index('__label__')
            tmp.index.name = summary.index.name
            return tmp.drop(columns=['__sort_key__'], errors='ignore')
        ...
        # so that recent-week slicing always follows the latest chronological order.
        ...
        if not weekly_summary.empty:
            try:
                ...
                sort_key = [
                    pd.Timestamp.fromisocalendar(int(y), max(1, min(int(w), 53)), 1)
                    for y, w in zip(idx_frame['year_num'], idx_frame['week_num'])
                ]
                weekly_summary = weekly_summary.copy()
                weekly_summary['_sort_key'] = sort_key
                weekly_summary = weekly_summary.sort_values('_sort_key')
                weekly_summary.index = [
                    f"{int(y)}-W{int(w):02d}" for y, w in zip(idx_frame['year_num'], idx_frame['week_num'])
                ]
        ...
        # Reorder weekly pivot chronologically using year-week labels.
        if not product_weekly_pivot.empty:
            sorted_week_index = sorted(product_weekly_pivot.index, key=_weekly_label_sort_key)
        ...
        def compute_combined_summaries(full_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
            ...
            if not weekly_combined.empty:
                weekly_combined['_sort_key'] = [
                    pd.Timestamp.fromisocalendar(int(y), max(1, min(int(w), 53)), 1)
                    for y, w in zip(
                        pd.to_numeric(weekly_combined['년도'], errors='coerce').fillna(1900).astype(int),
                        pd.to_numeric(weekly_combined['주차번호'], errors='coerce').fillna(1).astype(int),
                    )
                ]
                weekly_combined = weekly_combined.sort_values('_sort_key')
                weekly_combined['주차'] = [
                    f"{int(y)}-W{int(w):02d}" for y, w in zip(
                        pd.to_numeric(weekly_combined['년도'], errors='coerce').fillna(1900).astype(int),
                        pd.to_numeric(weekly_combined['주차번호'], errors='coerce').fillna(1).astype(int),
                    )
                ]
        ...
        with tab_weekly:
            ...
            if selected_date_weekly == '전체':
                ...
                iso_year = int(iso_cal.year)
                week_key = f"{iso_year}-W{week_no:02d}"
        ...
        if selected_period == "일별":
            ...
            pivot_source = _limit_recent_weeks_df(product_weekly_pivot, 10)
            index_label = "연-주"
            title_suffix = "주간"
        else:
        ...
        if not category_columns:
            ...
            if selected_period == "주간":
                pivot_filtered = _sort_limit_weekly_df(pivot_filtered, 10)
