...
def filter_root_cause_base(
    ...
    data = data[(data["생산일자"] >= pd.to_datetime(start)) & (data["생산일자"] <= pd.to_datetime(end))].copy()
    return data
...
def summarise_machine_day(df: pd.DataFrame, machine_col: str, selected_day) -> pd.DataFrame:
    ...
    grouped["불량비중"] = np.where(total_defect > 0, grouped["불량수량"] / total_defect * 100, 0)
...
def summarise_machine_defect_mix(
    ...
    """선택일+선택호기 기준 불량유형 구성만 집계합니다.

    양품수량은 불량유형별로 동일하게 반복될 수 있으므로 유형별 표에서는
    불량수량/불량비중/해당 호기 총생산 기준 불량율만 제공합니다.
    """
    data = df[(df["생산일자"].dt.date == pd.to_datetime(selected_day).date()) & (df[machine_col].astype(str) == str(selected_machine))].copy()
    if data.empty:
        return pd.DataFrame(columns=["불량유형", "불량수량", "불량비중", "호기전체불량율"])

    total_good = data["양품수량"].sum()
    total_defect = data["불량수량"].sum()
    total_prod = total_good + total_defect

    grouped = data.groupby("불량유형").agg(불량수량=("불량수량", "sum")).reset_index()
    grouped["불량비중"] = np.where(total_defect > 0, grouped["불량수량"] / total_defect * 100, 0)
    grouped["호기전체불량율"] = np.where(total_prod > 0, grouped["불량수량"] / total_prod * 100, 0)
...
def plot_daily_anomaly_chart(daily_df: pd.DataFrame, selected_day=None) -> go.Figure:
    fig = go.Figure()
    if daily_df.empty:
        fig.update_layout(title="일별 이상일 탐색")
        return fig
    ...
    fig.add_hline(y=avg_rate, line_dash="dash", line_color="black", annotation_text=f"평균 {avg_rate:.1f}%")
    fig.update_layout(title="일별 이상일 탐색", xaxis_title="날짜", yaxis_title="불량율 (%)", xaxis=dict(type="category"), height=420)
    return fig


def plot_machine_bar_chart(machine_df: pd.DataFrame, machine_col: str) -> go.Figure:
    fig = go.Figure()
    if machine_df.empty:
        fig.update_layout(title="호기별 불량율 분석")
        return fig

    avg_rate = machine_df["불량율"].mean()
    top_machine = machine_df.iloc[0][machine_col]
    colors = ["#d62728" if m == top_machine else "#6baed6" for m in machine_df[machine_col]]
    fig.add_trace(go.Bar(
        x=machine_df["불량율"],
        y=machine_df[machine_col].astype(str),
        orientation="h",
        marker_color=colors,
        text=[f"{r['불량율']:.1f}% / {int(r['불량수량'])}ea" for _, r in machine_df.iterrows()],
        textposition="outside",
        name="호기별 불량율",
    ))
    fig.add_vline(x=avg_rate, line_dash="dash", line_color="black", annotation_text=f"평균 {avg_rate:.1f}%")
    fig.update_layout(title="호기별 불량율 분석", xaxis_title="불량율 (%)", yaxis_title="공정기계코드", height=max(360, len(machine_df) * 45))
    return fig


def plot_machine_defect_chart(defect_df: pd.DataFrame, machine_name: str) -> go.Figure:
    if defect_df.empty:
        fig = go.Figure()
        fig.update_layout(title=f"{machine_name} 불량유형 구성")
        return fig
    ...
    fig = px.bar(
        ...
        text="불량수량",
        color="불량유형",
        title=f"{machine_name} 불량유형 구성",
    )
    fig.update_layout(xaxis_title="불량유형", yaxis_title="불량수량", showlegend=False, height=380)
    return fig
...
def render_root_cause_analysis(df: pd.DataFrame) -> None:
    st.markdown("---")
    st.markdown("## 이상 원인분석")
    st.caption("기간/공정을 고르면 불량율이 가장 높은 날짜와 호기를 자동으로 찾아 원인을 보여줍니다.")
    ...
    base_df = filter_root_cause_base(df, selected_plant, selected_process, date_mode, start_date, end_date)
    if base_df.empty:
        st.info("선택 조건에 해당하는 데이터가 없습니다.")
        return

    daily_df = summarise_daily_anomaly(base_df)
    if ranking_mode == "불량율 높은 순":
        select_source = daily_df.sort_values(["불량율", "불량수량"], ascending=[False, False])
    elif ranking_mode == "불량수량 높은 순":
        select_source = daily_df.sort_values(["불량수량", "불량율"], ascending=[False, False])
    else:
        select_source = daily_df.sort_values(["평균대비", "불량율"], ascending=[False, False])

    if select_source.empty:
        st.info("이상일 후보가 없습니다.")
        return

    selected_day = str(select_source.iloc[0]["날짜"])
    st.plotly_chart(plot_daily_anomaly_chart(daily_df, selected_day=selected_day), use_container_width=True)

    selected_day_row = daily_df[daily_df["날짜"].astype(str) == selected_day]
    if not selected_day_row.empty:
        row = selected_day_row.iloc[0]
        k1, k2, k3, k4 = st.columns(4)
        k1.metric("자동 선택 날짜", str(row["날짜"]))
    ...
    with left:
        st.plotly_chart(plot_machine_bar_chart(machine_df, machine_col), use_container_width=True)

    with right:
        if machine_df.empty:
            st.info("선택일 호기 데이터가 없습니다.")
            return
        selected_machine = str(machine_df.iloc[0][machine_col])
        st.markdown(f"**자동 선택 호기:** {selected_machine} (불량율 최고)")
    ...
    defect_mix_df = summarise_machine_defect_mix(base_df, machine_col, selected_day, selected_machine)
    dcol1, dcol2 = st.columns([1.0, 1.2])
    with dcol1:
        st.plotly_chart(plot_machine_defect_chart(defect_mix_df, selected_machine), use_container_width=True)
    with dcol2:
        if defect_mix_df.empty:
            st.info("최고 불량율 호기의 상세 불량유형 데이터가 없습니다.")
        else:
            view_df = defect_mix_df.copy()
            view_df["불량비중"] = view_df["불량비중"].map(lambda x: f"{x:.2f}")
            view_df["호기전체불량율"] = view_df["호기전체불량율"].map(lambda x: f"{x:.2f}")
            st.dataframe(view_df, use_container_width=True, hide_index=True)

    with st.expander("원본 상세 데이터"):
        raw_df = base_df[(base_df["생산일자"].dt.date == pd.to_datetime(selected_day).date()) & (base_df[machine_col].astype(str) == selected_machine)].copy()
...
def run_defect_dashboard():
    ...
    for proc_code, proc_label in process_mapping.items():
        ...
        with col1:
            ...
            fig = plot_time_series(pivot, f"{proc_label} 월별 불량율", categories, y_range=[0, 50])
        ...
        with col2:
            ...
            if len(pivot) > 10:
                pivot = pivot.tail(10)
            fig = plot_time_series(pivot, f"{proc_label} 주간 불량율", categories, y_range=[0, 50])
            st.plotly_chart(fig, use_container_width=True)
        # 일별 그래프 (최근 30일)
        with col3:
            ...
            if len(pivot) > 30:
                pivot = pivot.tail(30)
            fig = plot_time_series(pivot, f"{proc_label} 일별 불량율", categories, y_range=[0, 50])
            st.plotly_chart(fig, use_container_width=True)
    # ────────────────────────────────────── 유형별 불량율 ──────────────────────────────────────
    st.markdown("## 유형별 불량율")
    ...
    for defect in defect_types_general:
        ...
        with c1:
            ...
            fig = plot_time_series(pivot.rename(columns=process_mapping), f"{defect} 월별 불량율", compare_process_labels, y_range=[0, 50])
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            pivot = summarise_by_time(df, 'W', compare_process_names, defect_filter=defect)
            if len(pivot) > 10:
                pivot = pivot.tail(10)
            fig = plot_time_series(pivot.rename(columns=process_mapping), f"{defect} 주간 불량율", compare_process_labels, y_range=[0, 50])
            st.plotly_chart(fig, use_container_width=True)
        with c3:
            pivot = summarise_by_time(df, 'D', compare_process_names, defect_filter=defect)
            if len(pivot) > 30:
                pivot = pivot.tail(30)
            fig = plot_time_series(pivot.rename(columns=process_mapping), f"{defect} 일별 불량율", compare_process_labels, y_range=[0, 50])
            st.plotly_chart(fig, use_container_width=True)
