...
def calc_defect_rate(good_qty: float, defect_qty: float) -> float:
    ...
    if total == 0:
        return 0.0
    return float(defect_qty / total * 100)
...
def filter_root_cause_base(
    ...
    max_date = pd.to_datetime(data["생산일자"], errors="coerce").max()
    if pd.isna(max_date):
        return data.iloc[0:0]

    if date_mode == "이번주":
        start = max_date.normalize() - pd.Timedelta(days=6)
        end = max_date.normalize()
    elif date_mode == "최근 2주":
        start = max_date.normalize() - pd.Timedelta(days=13)
        end = max_date.normalize()
    elif date_mode == "최근 4주":
        start = max_date.normalize() - pd.Timedelta(days=27)
        end = max_date.normalize()
    else:
    ...
    data = data[(data["생산일자"] >= start) & (data["생산일자"] <= end)].copy()
    return data
...
def summarise_daily_anomaly(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped["불량율"] = grouped.apply(
        lambda r: calc_defect_rate(r["양품수량"], r["불량수량"]), axis=1
    )
...
def summarise_machine_day(df: pd.DataFrame, machine_col: str, selected_day) -> pd.DataFrame:
    ...
    day_df = df[df["생산일자"].dt.date == day].copy()
    if day_df.empty:
    ...
    grouped = grouped.sort_values(["불량율", "불량수량"], ascending=[False, False])
    return grouped
...
def summarise_machine_defect_mix(
    ...
    if data.empty:
        return pd.DataFrame(columns=["불량유형", "양품수량", "불량수량", "생산수량", "불량율"])
    ...
    grouped = grouped.sort_values(["불량수량", "불량율"], ascending=[False, False])
    return grouped
...
def plot_daily_anomaly_chart(daily_df: pd.DataFrame, selected_day=None) -> go.Figure:
    ...
    avg_rate = float(daily_df["불량율"].mean())
    colors = []
...
def plot_machine_bar_chart(machine_df: pd.DataFrame, machine_col: str) -> go.Figure:
    ...
    avg_rate = float(machine_df["불량율"].mean())
    ...
    fig.update_layout(
        ...
        xaxis_title="불량율 (%)",
        yaxis_title="공정기계코드",
        height=max(360, len(machine_df) * 45),
    )
    return fig
...
def render_root_cause_analysis(df: pd.DataFrame) -> None:
    """이상일 원인분석 UI 렌더링."""
    st.markdown("---")
    st.markdown("## 이상일 원인분석")
    st.caption("불량율이 높았던 날짜를 선택해 공정기계코드(호기)별 원인을 확인합니다.")

    machine_col = resolve_machine_col(df)
    if machine_col is None:
        st.warning("공정기계코드(호기) 관련 컬럼이 없어 원인분석 화면은 표시하지 않습니다.")
        return
    ...
    if daily_df.empty:
        st.info("분석 가능한 일별 데이터가 없습니다.")
        return

    if ranking_mode == "불량율 높은 순":
        select_source = daily_df.sort_values("불량율", ascending=False)
    elif ranking_mode == "불량수량 높은 순":
        select_source = daily_df.sort_values("불량수량", ascending=False)
    else:
        select_source = daily_df.sort_values("평균대비", ascending=False)

    day_options = select_source["날짜"].astype(str).tolist()
    selected_day = st.selectbox("분석할 날짜 선택", day_options, key="rc_selected_day")
    ...
    if not selected_day_row.empty:
        ...
        machine_df_preview = summarise_machine_day(base_df, machine_col, selected_day)
        top_machine = str(machine_df_preview.iloc[0][machine_col]) if not machine_df_preview.empty else "-"
        k1, k2, k3, k4 = st.columns(4)
        k1.metric("선택일", str(row["날짜"]))
        k2.metric("당일 불량율", f"{row['불량율']:.2f}%")
        k3.metric("불량수량", f"{int(row['불량수량']):,}")
        k4.metric("최다 불량 호기", top_machine)
    ...
    with right:
        ...
        selected_machine = st.selectbox("상세 분석할 호기 선택", machine_options, key="rc_machine")
    ...
    with dcol1:
        st.plotly_chart(plot_machine_defect_chart(defect_mix_df), use_container_width=True)
    with dcol2:
        if defect_mix_df.empty:
            st.info("선택 호기의 상세 불량유형 데이터가 없습니다.")
        else:
            view_df = defect_mix_df.copy()
            view_df["불량율"] = view_df["불량율"].map(lambda x: f"{x:.2f}")
