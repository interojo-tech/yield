...
def load_data(file_path: str, file_mtime: float) -> pd.DataFrame:
    ...
    Args:
        ...
        file_mtime: 엑셀 파일의 마지막 수정 시간 (유닉스 타임스탬프). 이 값을
            인자로 사용하여 Streamlit 캐시를 무효화합니다. 파일이 갱신되면
            수정 시간이 달라지므로 캐시가 자동으로 갱신됩니다.
...
def run_defect_dashboard():
    ...
    if file_path is None:
        ...
        return
    # 파일 수정 시간(Unix timestamp)을 읽어 캐시 무효화 키로 사용합니다.
    try:
    ...
    # ----------------------------------------------------------------------
    # 공정 및 불량 유형 정의 (동적 생성)
    # 기존에는 공정명과 불량 유형을 하드코딩했지만, 엑셀 데이터에 새로운 공정이나
    # 불량 유형이 추가될 수 있기 때문에 데이터로부터 자동으로 추출합니다.
    processes = sorted(df["공정명"].dropna().unique().tolist())
    # 공정명 표시 라벨을 생성합니다. 공정명이 '공정' 또는 '검사'로 끝나지 않으면 '공정'을 붙여줍니다.
    process_mapping = {p: (p if p.endswith('공정') or p.endswith('검사') else f"{p}공정") for p in processes}
    # 각 공정별 불량 유형 목록을 수집합니다.
    categories_by_process = {p: sorted(df[df["공정명"] == p]["불량유형"].dropna().unique().tolist()) for p in processes}
    # 전체 불량 유형 목록
    all_defects = sorted(df["불량유형"].dropna().unique().tolist())
    # 누수/규격검사 공정의 불량 유형 목록 (없으면 빈 리스트)
    defect_types_leak = categories_by_process.get('누수/규격검사', [])
    # 일반 공정에서 나타낼 불량 유형 목록: 전체 목록에서 누수/규격검사 공정의 불량 유형 제외
    defect_types_general = [d for d in all_defects if d not in defect_types_leak]
    # 공정별 비교에 사용할 공정명 및 라벨 목록 (누수/규격검사 제외)
    compare_process_names = [p for p in processes if p != '누수/규격검사']
    ...
    # 공정별 비교에 사용할 공정명 및 라벨 목록은 앞에서 동적으로 계산된 compare_process_names/labels를 사용합니다.
    # 일반 불량 유형에 대한 그래프
