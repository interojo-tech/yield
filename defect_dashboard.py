...
def load_data(file_path: str) -> pd.DataFrame:
    ...
    # - 불량수량1: 상세 데이터의 실적/호기 총 불량수량 기준 컬럼
    # - 불량수량_실적: 공정/집계용 실제 불량수량(가능하면 `불량수량1` 계열 우선 사용)
    df["불량수량_유형"] = pd.to_numeric(df.get("불량수량", 0), errors='coerce').fillna(0)
    if "불량수량1" in df.columns:
        df["불량수량1"] = pd.to_numeric(df["불량수량1"], errors='coerce').fillna(0)
    elif "불량수량.1" in df.columns:
        df["불량수량1"] = pd.to_numeric(df["불량수량.1"], errors='coerce').fillna(0)
    else:
        df["불량수량1"] = df["불량수량_유형"]
    df["불량수량_실적"] = df["불량수량1"]
...
def summarise_machine_defect_mix(
    ...
    if "불량수량1" in data.columns:
        defect_qty_col = "불량수량1"
    elif "불량수량.1" in data.columns:
        defect_qty_col = "불량수량.1"
    elif "불량수량_유형" in data.columns:
        defect_qty_col = "불량수량_유형"
    else:
        defect_qty_col = "불량수량"
...
def render_root_cause_analysis(df: pd.DataFrame) -> None:
    ...
    with dcol2:
        if defect_mix_df.empty:
            ...
            machine_defect_col = "불량수량1" if "불량수량1" in machine_raw_df.columns else "불량수량"
            machine_defect_actual = pd.to_numeric(machine_raw_df[machine_defect_col], errors="coerce").fillna(0).sum()
