...
def load_data(file_path: str) -> pd.DataFrame:
    ...
    # 불량수량 컬럼 보존:
    # - 불량수량_실적: 수율/불량율 계산용 전체 불량수량 (원본 `불량수량` 우선)
    # - 불량수량_상세: 불량명 옆 상세 분해 수량 (`불량수량1` 또는 `불량수량.1`)
    #
    # 주의:
    # 이상 원인분석의 불량율은 전체 생산수량 대비 전체 불량수량으로 계산되어야 하므로
    # `불량수량1`이 아니라 원본 `불량수량`을 기준으로 잡습니다.
    # 반면 선택 호기 불량유형 구성 표/그래프는 사용자가 확인하는 상세 컬럼인
    # `불량수량1` 계열을 사용합니다.
    df["불량수량_실적"] = pd.to_numeric(df.get("불량수량", 0), errors='coerce').fillna(0)
    if "불량수량1" in df.columns:
        df["불량수량_상세"] = pd.to_numeric(df["불량수량1"], errors='coerce').fillna(0)
    elif "불량수량.1" in df.columns:
        df["불량수량_상세"] = pd.to_numeric(df["불량수량.1"], errors='coerce').fillna(0)
    else:
        df["불량수량_상세"] = df["불량수량_실적"]
...
def summarise_by_time(
    ...
    """일/주/월 단위 불량율 피벗 테이블을 생성합니다.

    분모는 생산실적 단위(날짜/공정/호기/품명 등)에서 중복 제거한 생산수량을 사용하고,
    분자는 불량유형 상세수량(`불량수량1` 계열)을 사용합니다.
    """
    base = df.copy()
    if process_filter is not None:
        base = base[base["공정명"] == process_filter]
    if group_filter is not None:
        base = base[base["공정명"].isin(group_filter)]
    if base.empty:
        return pd.DataFrame(columns=categories)

    units = _add_period_column(get_production_unit_df(base), time_unit)
    num = _add_period_column(base, time_unit)
    detail_col = get_detail_defect_col(num)

    if defect_filter is not None:
        num = num[num["불량유형"] == defect_filter].copy()
        numer = num.groupby(["기간", "공정명"])[detail_col].sum().reset_index(name="불량수량")
        denom = units.groupby(["기간", "공정명"])["생산수량"].sum().reset_index(name="생산수량")
        agg = numer.merge(denom, on=["기간", "공정명"], how="left")
        agg["불량율"] = np.where(agg["생산수량"] > 0, agg["불량수량"] / agg["생산수량"] * 100, 0.0)
        pivot_col = "공정명"
    else:
        numer = num.groupby(["기간", "불량유형"])[detail_col].sum().reset_index(name="불량수량")
        denom = units.groupby(["기간"])["생산수량"].sum().reset_index(name="생산수량")
        agg = numer.merge(denom, on=["기간"], how="left")
        agg["불량율"] = np.where(agg["생산수량"] > 0, agg["불량수량"] / agg["생산수량"] * 100, 0.0)
        pivot_col = "불량유형"
    ...
    if time_unit == 'W':
        ...
        pivot.index = [month_map.get(int(idx), str(idx)) for idx in pivot.index]
    else:
        pivot.sort_index(inplace=True)
    return pivot


def get_total_defect_col(df: pd.DataFrame) -> str:
    """전체 불량수량 컬럼명을 반환합니다."""
    for col in ["불량수량_실적", "불량수량"]:
        if col in df.columns:
            return col
    return "불량수량"


def get_detail_defect_col(df: pd.DataFrame) -> str:
    """불량유형 상세수량 컬럼명을 반환합니다."""
    for col in ["불량수량_상세", "불량수량1", "불량수량.1", "불량수량"]:
        if col in df.columns:
            return col
    return "불량수량"


def get_production_unit_df(df: pd.DataFrame) -> pd.DataFrame:
    """불량유형 행 중복을 제거한 생산실적 단위 데이터프레임을 반환합니다."""
    if df.empty:
        return df.copy()
    detail_cols = {
        "불량명", "불량유형", "불량코드", "불량수량1", "불량수량.1",
        "불량수량_유형", "불량수량_상세", "점유율"
    }
    keep_cols = [c for c in df.columns if c not in detail_cols]
    units = df[keep_cols].drop_duplicates().copy()
    total_col = get_total_defect_col(units)
    units["양품수량"] = pd.to_numeric(units.get("양품수량", 0), errors="coerce").fillna(0)
    units["불량수량"] = pd.to_numeric(units.get(total_col, 0), errors="coerce").fillna(0)
    units["생산수량"] = units["양품수량"] + units["불량수량"]
    units["불량율"] = np.where(units["생산수량"] > 0, units["불량수량"] / units["생산수량"] * 100, 0.0)
    return units


def _add_period_column(df: pd.DataFrame, time_unit: str) -> pd.DataFrame:
    data = df.copy()
    if time_unit == 'D':
...
def summarise_daily_anomaly(df: pd.DataFrame) -> pd.DataFrame:
    ...
    units = get_production_unit_df(df)
    grouped = (
        units.groupby(units["생산일자"].dt.date)
...
def summarise_machine_day(df: pd.DataFrame, machine_col: str, selected_day) -> pd.DataFrame:
    ...
    units = get_production_unit_df(day_df)
    grouped = (
        units.groupby(machine_col)
        .agg(양품수량=("양품수량", "sum"), 불량수량=("불량수량", "sum"))
        .reset_index()
    )
    grouped["생산수량"] = grouped["양품수량"] + grouped["불량수량"]
    ...
    return grouped.sort_values(["불량율", "불량수량"], ascending=[False, False]).reset_index(drop=True)
...
def summarise_machine_defect_mix(
    ...
    불량유형 구성에서는 불량명 옆의 `불량수량1` 계열 컬럼을 기준으로
    유형별 불량수량을 집계합니다. 원본 상세 데이터에서 사용자가 확인하는
    기준과 동일하게 맞추기 위해 `불량수량1` 또는 `불량수량.1`을 우선
    사용하고, 없을 때만 기존 `불량수량_유형` 또는 `불량수량`으로 대체합니다.
    """
    data = df[
    ...
    if "불량수량_상세" in data.columns:
        defect_qty_col = "불량수량_상세"
    elif "불량수량1" in data.columns:
        defect_qty_col = "불량수량1"
    elif "불량수량.1" in data.columns:
        defect_qty_col = "불량수량.1"
    else:
        defect_qty_col = "불량수량"
...
def plot_machine_defect_chart(defect_df: pd.DataFrame) -> go.Figure:
    ...
    plot_df = defect_df.copy()
    if "불량유형" in plot_df.columns:
        plot_df = plot_df[plot_df["불량유형"].astype(str) != "총합"].copy()

    if plot_df.empty:
        fig.update_layout(title="선택 호기 불량유형 구성")
        return fig

    x_vals = plot_df["불량유형"].astype(str).tolist()
    qty_vals = pd.to_numeric(plot_df["불량수량"], errors="coerce").fillna(0)
    ratio_vals = pd.to_numeric(plot_df["불량비중"], errors="coerce").fillna(0)
    ...
    fig.add_trace(
        go.Bar(
            ...
            y=qty_vals,
            name="불량수량",
            marker=dict(color="#9ecae1", line=dict(color="#4a90c2", width=1.2)),
            text=[f"{int(v):,}" for v in qty_vals],
            textposition="outside",
            textfont=dict(color="#2f5f85", size=12),
            opacity=0.9,
            yaxis="y2",
        )
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            text=[f"<b>{v:.1f}%</b>" for v in ratio_vals],
            textposition="top center",
            textfont=dict(color="#b30000", size=13),
            yaxis="y",
        )
    )
    ...
    fig.update_layout(
        title="선택 호기 불량유형 구성",
        xaxis=dict(title="불량유형", type="category", categoryorder="array", categoryarray=x_vals),
        yaxis=dict(
            title="불량비중 (%)",
            rangemode="tozero",
            side="left",
...
def render_root_cause_analysis(df: pd.DataFrame) -> None:
    ...
    with dcol2:
        if defect_mix_df.empty:
            ...
            machine_units = get_production_unit_df(machine_raw_df)
            machine_good = pd.to_numeric(machine_units["양품수량"], errors="coerce").fillna(0).sum()
            machine_defect_actual = pd.to_numeric(machine_units["불량수량"], errors="coerce").fillna(0).sum()
