...
def load_data(file_path: str, file_mtime: float) -> pd.DataFrame:
    ...
    Args:
        ...
        file_mtime: 엑셀 파일의 마지막 수정 시간 (유닉스 타임스탬프). 이 값을
            캐시 키에 포함하여, 엑셀 파일이 수정되면 캐시가 자동으로 무효화되도록 합니다.
...
def run_defect_dashboard():
    ...
    if file_path is None:
        ...
        return
    # 파일의 마지막 수정 시간을 읽어 캐시 키로 사용합니다. 파일이 업데이트되면 수정 시간이 바뀌므로 캐시가 자동으로 갱신됩니다.
    try:
    ...
    # 공정명 라벨링: '공정'이나 '검사'로 끝나지 않으면 '공정'을 추가
    ...
    # 일반 불량 유형 목록: 전체 목록에서 누수/규격검사 공정 불량을 제외
