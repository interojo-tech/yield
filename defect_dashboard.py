...
def run_defect_dashboard():
    ...
    data_files: List[str] = []
    # 우선 현재 디렉터리에서 '(26.xx)불량실적현황.xlsx' 패턴을 찾습니다.
    for fname in os.listdir('.'):
        if fname.endswith('.xlsx') and '불량실적현황' in fname:
            data_files.append(fname)
    # 또한 'data' 서브디렉터리가 있다면 그 안에서도 찾습니다.
    if os.path.isdir('data'):
        for fname in os.listdir('data'):
            if fname.endswith('.xlsx') and '불량실적현황' in fname:
                data_files.append(os.path.join('data', fname))
    # 만약 월별 파일을 찾지 못했다면 기존 후보 파일을 사용합니다.
    if not data_files:
        candidates = [
            "공정기술팀 대시보드(불량).xlsx",
            "공정기술팀 대시보드 (불량).xlsx",
        ...
        for c in candidates:
            ...
            if os.path.exists(os.path.join('data', c)):
                data_files.append(os.path.join('data', c))
                break
    if not data_files:
        st.error("불량 데이터 파일을 찾을 수 없습니다. 불량실적현황 엑셀 파일을 프로젝트에 업로드해 주세요.")
        return
    # 모든 파일을 로드 후 결합
    ...
    for proc_code, proc_label in process_mapping.items():
        ...
        # 월별 그래프
        with col1:
            pivot = summarise_by_time(df, 'M', categories, process_filter=proc_code, defect_filter=selected_defect_type)
            fig = plot_time_series(pivot, f"{proc_label} 월별 불량율", categories, y_range=[0, 50])
            st.plotly_chart(fig, use_container_width=True)
        # 주간 그래프 (최근 10주)
        with col2:
            pivot = summarise_by_time(df, 'W', categories, process_filter=proc_code, defect_filter=selected_defect_type)
            if len(pivot) > 10:
                pivot = pivot.tail(10)
        ...
        # 일별 그래프 (최근 30일)
        with col3:
            pivot = summarise_by_time(df, 'D', categories, process_filter=proc_code, defect_filter=selected_defect_type)
            if len(pivot) > 30:
                pivot = pivot.tail(30)
