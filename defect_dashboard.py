...
def run_defect_dashboard():
    ...
    # 데이터 경로 탐색
    # 불량 데이터 파일명 패턴: (26.xx)불량실적현황.xlsx 또는 26.xx불량실적현황.xlsx,
    # where xx is a zero‑padded month from 01 to 12.  We search for
    # files starting from the most recent month (12) down to 01.
    ...
    for m in range(12, 0, -1):
        ...
        month_candidates.extend([
            f"({code})불량실적현황.xlsx",
            f"{code}불량실적현황.xlsx",
            f"({code}) 불량실적현황.xlsx",  # allow a space after parenthesis
            f"{code} 불량실적현황.xlsx",
        ])
    ...
    for c in month_candidates:
        ...
        if os.path.exists(c):
            file_path = c
            break
        # Also check in a 'data' subdirectory
        data_path = os.path.join("data", c)
        if os.path.exists(data_path):
            file_path = data_path
            break
    if file_path is None:
        st.error(
            "불량 데이터 파일을 찾을 수 없습니다. '(26.xx)불량실적현황.xlsx' 형식의 파일을 프로젝트에 업로드해 주세요."
        )
        return
    # 파일 수정 시각을 포함하여 캐시 키를 생성합니다. 엑셀 파일이 업데이트되면 캐시가 자동 무효화됩니다.
    @st.cache_data(show_spinner=False)
    def _load_data_cached(path: str, mtime: float) -> pd.DataFrame:
        return load_data(path)

    df = _load_data_cached(file_path, os.path.getmtime(file_path))
    if df.empty:
        st.warning("데이터를 불러올 수 없거나 필수 컬럼이 없습니다.")
        return
    # ----------------------------------------------------------------------
    # 공정 및 불량 유형 정의 (고정)
    # 원본 구현에서는 공정명과 불량 유형을 하드코딩하여 대시보드에 표시했습니다.
    # 엑셀 파일에 새로운 공정/불량이 추가되더라도 대시보드 구조가 변하지 않도록 기존 리스트를 유지합니다.
    ...
    # 일반 공정 목록(누수/규격검사 제외)과 라벨을 정의합니다.
    compare_process_names = ["사출조립", "분리", "접착/멸균"]
    compare_process_labels = [process_mapping[p] for p in compare_process_names]
    # 불량 유형을 고정된 목록으로 정의합니다. 누수/규격검사 공정은 별도로 분리합니다.
    defect_types_general = ["파손", "엣지기포", "엣지", "미분리", "뜯김"]
    defect_types_leak = ["리드지 불량", "블리스터 불량"]
    # 공정별 불량 유형 매핑을 정의합니다. 사출조립/분리/접착·멸균 공정은 일반 불량 유형을 사용하고,
    # 누수/규격검사 공정은 누수 관련 불량 유형을 사용합니다.
    categories_by_process = {
        "사출조립": defect_types_general,
        "분리": defect_types_general,
        "접착/멸균": defect_types_general,
        "누수/규격검사": defect_types_leak,
    }
    # ────────────────────────────────────── 공정별 불량율 ─────────────────────────────────────
    st.markdown("## 공정별 불량율")
    ...
    for proc_code, proc_label in process_mapping.items():
        ...
        # 공정별 사용할 불량 유형 목록을 데이터에서 추출합니다. 해당 공정의 불량 유형이 없으면 빈 리스트를 사용합니다.
        categories = categories_by_process.get(proc_code, [])
    ...
    # 공정별 비교에 사용할 공정명 및 라벨 목록은 위에서 동적으로 계산되었습니다.
    # 일반 불량 유형에 대한 그래프
