...
def load_data(file_path: str, mtime: float) -> pd.DataFrame:
    """
    ...
    df = df.sort_values(by="생산일자").reset_index(drop=True)
    return df
...
def run_defect_dashboard():
    ...
    if file_path is None:
        ...
        return
    # Pass file modification time to invalidate cache when data updates
