...
def run_defect_dashboard():
    ...
    for defect in defect_types_general:
        ...
        with c1:
            ...
            fig = plot_time_series(pivot.rename(columns=process_mapping), f"{defect} 월별 불량율", compare_process_labels, y_range=[0, 100])
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            pivot = summarise_by_time(df, 'W', compare_process_names, defect_filter=defect)
            if len(pivot) > 10:
                pivot = pivot.tail(10)
            fig = plot_time_series(pivot.rename(columns=process_mapping), f"{defect} 주간 불량율", compare_process_labels, y_range=[0, 100])
            st.plotly_chart(fig, use_container_width=True)
        with c3:
            pivot = summarise_by_time(df, 'D', compare_process_names, defect_filter=defect)
            if len(pivot) > 30:
                pivot = pivot.tail(30)
            fig = plot_time_series(pivot.rename(columns=process_mapping), f"{defect} 일별 불량율", compare_process_labels, y_range=[0, 100])
            st.plotly_chart(fig, use_container_width=True)
    ...
    for defect in defect_types_leak:
        ...
        with c1:
            ...
            fig = plot_time_series(pivot.rename(columns={proc_name: proc_label}), f"{defect} 월별 불량율", [proc_label], y_range=[0, 100])
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            pivot = summarise_by_time(df, 'W', [proc_name], defect_filter=defect, process_filter=proc_name)
            if len(pivot) > 10:
                pivot = pivot.tail(10)
            fig = plot_time_series(pivot.rename(columns={proc_name: proc_label}), f"{defect} 주간 불량율", [proc_label], y_range=[0, 100])
            st.plotly_chart(fig, use_container_width=True)
        with c3:
            pivot = summarise_by_time(df, 'D', [proc_name], defect_filter=defect, process_filter=proc_name)
            if len(pivot) > 30:
                pivot = pivot.tail(30)
            fig = plot_time_series(pivot.rename(columns={proc_name: proc_label}), f"{defect} 일별 불량율", [proc_label], y_range=[0, 100])
            st.plotly_chart(fig, use_container_width=True)
