...
def summarise_machine_defect_mix(
    ...
    주의: 양품수량은 개별 불량유형에 1:1로 귀속되지 않는 경우가 많아
    유형별 표에서는 반복 표기하지 않습니다. 대신 불량수량, 불량비중,
    해당 호기의 전체 불량율만 보여줍니다.
    """
    data = df[
    ...
    machine_good_qty = data["양품수량"].sum()
    machine_defect_qty = data["불량수량"].sum()
    machine_rate = calc_defect_rate(machine_good_qty, machine_defect_qty)
...
def plot_machine_bar_chart(machine_df: pd.DataFrame, machine_col: str) -> go.Figure:
    ...
    colors = [
        "#d62728" if m == top_machine else "#6baed6"
        for m in machine_df[machine_col]
    ]
...
def render_root_cause_analysis(df: pd.DataFrame) -> None:
    ...
    st.markdown("---")
    st.markdown("## 이상 원인분석")
    st.caption("현재 조건에서 이상치로 판단된 날짜를 자동 선택해 호기별 원인을 확인합니다.")
    ...
    with left:
        st.plotly_chart(
            plot_machine_bar_chart(machine_df, machine_col),
            use_container_width=True,
        )
    ...
    with right:
        ...
        default_machine = machine_options[0] if machine_options else None
        ...
        if default_machine is not None:
            st.caption(f"자동 기본 호기: {default_machine} (불량율 최고)")
