...
def summarise_machine_defect_mix(
    ...
    machine_good = pd.to_numeric(data["양품수량"], errors="coerce").fillna(0).sum()
    machine_defect_actual = pd.to_numeric(data["불량수량"], errors="coerce").fillna(0).sum()
...
def render_root_cause_analysis(df: pd.DataFrame) -> None:
    ...
    st.markdown("---")
    st.markdown("## 이상 원인분석")
    st.caption("조건 내 이상일을 자동 선택해 호기별 원인을 확인합니다.")
    ...
    selected_day = day_options[0] if day_options else None
    ...
    with right:
        ...
        selected_machine = st.selectbox("상세 분석할 호기 선택 (기본값: 불량율 최고 호기)", machine_options, index=0, key="rc_machine")
