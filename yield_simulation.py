...
def main() -> None:
    ...
    if st:
        ...
        # Provide the dashboard selection radio without an extra wrapper.  Each
        # radio option is styled via the CSS injected earlier.  The label
        # argument acts as an accessible name but is hidden by our CSS.
        main_tab = st.radio(
            "대시보드 선택",
            ["수율 대시보드", "불량 대시보드"],
            horizontal=True,
        ...
        def draw_line_chart(summary: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                fig = px.line(
                    ...
                    # Format the numeric value as text with one decimal place for data labels
                    text=df_long["수율"].map(lambda v: f"{v:.1f}"),
                )
        ...
        def draw_product_chart(pivot_df: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # Format the numeric value for data labels to show one decimal place
                fig.update_traces(text=[f"{v:.1f}" for v in df_long["수율"]], textposition="top center")
                st.plotly_chart(fig, use_container_width=True)
            else:
