...
def main() -> None:
    ...
    if st:
        ...
        # Update the main title.  Use a concise title without the word "수율" so
        # that it reads "공정별 대시보드" instead of the older "공정별 수율 대시보드".
        st.title("공정별 대시보드")
        ...
        def draw_line_chart(summary: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # Move the y-axis starting point to 50 and limit to 100.  This ensures
                # all yield graphs show the range 50–100 instead of starting at 0.
                ...
                try:
                    ...
                    pass
                # Set y-axis range to 50–100 for consistency with Plotly charts
                ax.set_ylim(50, 100)
        ...
        # ------------------------------------------------------------------
        # Compute combined yield and good quantity metrics for monthly and weekly
        # views.  These summaries aggregate across all processes to provide a
        # holistic view of yield by plant and overall yield along with the total
        # good quantity.  The resulting DataFrames have index labels that
        # correspond to the time period (e.g. 'YYYY-월' for monthly and 'W#'
        # for weekly) and columns representing the good quantity ('양품량'),
        # plant yields (A관, C관, S관) and overall yield ('종합수율').
        def compute_combined_summaries(data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
            """Return monthly and weekly combined summaries.

            Parameters
            ----------
            data : pd.DataFrame
                Filtered production DataFrame containing columns '년도', '월',
                '주차', '공장', '생산수량' and '양품수량'.

            Returns
            -------
            Tuple[pd.DataFrame, pd.DataFrame]
                (monthly_summary, weekly_summary) DataFrames with columns
                ['양품량', 'A관', 'C관', 'S관', '종합수율'] and index labels as
                described above.
            """
            # Helper to extract plant code (A, C, S) from the full plant name
            def _short_plant(name: str) -> str:
                # e.g. 'A관(1공장)' -> 'A관'; take substring up to first '관'
                try:
                    return name.split("관")[0] + "관"
                except Exception:
                    # fallback to first character + '관' if split fails
                    return name[0] + "관"

            # Monthly aggregation
            # Group by year, month and plant to compute totals
            monthly_group = (
                data.groupby(["년도", "월", "공장"])
                .agg(prod_qty=("생산수량", "sum"), good_qty=("양품수량", "sum"))
                .reset_index()
            )
            # Compute yield per plant for each month (in percent)
            monthly_group["yield"] = (
                monthly_group["good_qty"] / monthly_group["prod_qty"] * 100.0
            )
            # Pivot yields by plant; columns become plant codes (A관, C관, S관)
            monthly_pivot = monthly_group.pivot_table(
                index=["년도", "월"],
                columns="공장",
                values="yield",
                aggfunc="first",
            )
            # Sum of good quantity across plants for each month
            monthly_good = monthly_group.groupby(["년도", "월"])["good_qty"].sum()
            # Overall yield per month: total good quantity / total production quantity
            monthly_overall = monthly_group.groupby(["년도", "월"]).apply(
                lambda df_: (df_["good_qty"].sum() / df_["prod_qty"].sum()) * 100.0
            )
            # Assemble monthly combined DataFrame
            monthly_comb = pd.DataFrame(index=monthly_good.index)
            monthly_comb["양품량"] = monthly_good
            # Add plant yields, renaming columns via _short_plant
            for col in monthly_pivot.columns:
                # Short name (e.g. 'A관' or original if already short)
                short_name = _short_plant(col)
                monthly_comb[short_name] = monthly_pivot[col]
            monthly_comb["종합수율"] = monthly_overall
            # Convert index to 'YYYY-월' string.  If the index is a MultiIndex
            # with two levels (year, month) use those levels; otherwise cast to
            # string.  This avoids errors if the index is not a MultiIndex.
            try:
                if isinstance(monthly_comb.index, pd.MultiIndex):
                    monthly_comb.index = monthly_comb.index.map(
                        lambda idx: f"{int(idx[0])}-{int(idx[1])}"
                    )
                else:
                    monthly_comb.index = monthly_comb.index.astype(str)
            except Exception:
                # Fallback: cast index to string
                monthly_comb.index = monthly_comb.index.astype(str)
            monthly_comb = monthly_comb.sort_index()
            # Weekly aggregation
            weekly_df = data.copy()
            # Ensure '주차' numeric component is extracted
            weekly_df["_week_num"] = (
                weekly_df["주차"].astype(str).str.extract(r"(\d+)")[0].astype(int)
            )
            weekly_df["year_week"] = (
                weekly_df["년도"].astype(int).astype(str) + "-" + weekly_df["_week_num"].astype(str)
            )
            weekly_group = (
                weekly_df.groupby(["year_week", "공장"])
                .agg(prod_qty=("생산수량", "sum"), good_qty=("양품수량", "sum"))
                .reset_index()
            )
            weekly_group["yield"] = (
                weekly_group["good_qty"] / weekly_group["prod_qty"] * 100.0
            )
            weekly_pivot = weekly_group.pivot_table(
                index="year_week",
                columns="공장",
                values="yield",
                aggfunc="first",
            )
            weekly_good = weekly_group.groupby("year_week")["good_qty"].sum()
            weekly_overall = weekly_group.groupby("year_week").apply(
                lambda df_: (df_["good_qty"].sum() / df_["prod_qty"].sum()) * 100.0
            )
            weekly_comb = pd.DataFrame(index=weekly_good.index)
            weekly_comb["양품량"] = weekly_good
            for col in weekly_pivot.columns:
                short_name = _short_plant(col)
                weekly_comb[short_name] = weekly_pivot[col]
            weekly_comb["종합수율"] = weekly_overall
            # Reorder weekly index by numeric week number, and label as W#
            try:
                week_nums = weekly_comb.index.to_series().str.extract(r"-(\d+)")[0].astype(int)
                order = week_nums.argsort()
                weekly_comb = weekly_comb.iloc[order.values]
                new_labels = [f"W{n}" for n in week_nums.iloc[order].values]
                weekly_comb.index = new_labels
            except Exception:
                # Fallback: keep existing order and prefix with 'W'
                weekly_comb.index = [f"W{w.split('-')[-1]}" for w in weekly_comb.index]
            return monthly_comb, weekly_comb

        def draw_combined_chart(combined_df: pd.DataFrame, title: str, xlabel: str) -> None:
            """Draw a combined bar/line chart showing good quantity and yields.

            Parameters
            ----------
            combined_df : pd.DataFrame
                DataFrame with columns: '양품량', 'A관', 'C관', 'S관', '종합수율'.
                The index is the time period label (e.g., 'YYYY-월' or 'W#').
            title : str
                Chart title.
            xlabel : str
            ...
            if USE_PLOTLY:
                ...
                x_vals = combined_df.index.tolist()
                # Bar for good quantity on secondary y-axis (right)
                fig.add_trace(
                    go.Bar(
                        x=x_vals,
                        y=combined_df["양품량"],
                        name="양품량",
                        marker_color="#5470c6",
                        text=combined_df["양품량"],
                        textposition="outside",
                    ),
                    secondary_y=True,
                )
                # Lines for each plant yield and overall yield on primary y-axis (left)
                # Determine which yield columns are present (A관, C관, S관 may be missing if plant filtered)
                for col_name in ["A관", "C관", "S관", "종합수율"]:
                    if col_name in combined_df.columns:
                        fig.add_trace(
                            go.Scatter(
                                x=x_vals,
                                y=combined_df[col_name],
                                name=col_name,
                                mode="lines+markers",
                                text=combined_df[col_name].round(2),
                ...
                # Bar on secondary axis for good quantity
                ax2 = ax1.twinx()
                ax2.bar(
                    x,
                    combined_df["양품량"],
                ...
                # Plot yields on primary axis
                markers = ["o", "s", "^", "d"]
                for idx, col_name in enumerate(["A관", "C관", "S관", "종합수율"]):
                    if col_name in combined_df.columns:
                        ax1.plot(
                            x,
                        ...
                        # Annotate points
                        for xi, yi in enumerate(combined_df[col_name]):
                            ax1.text(
                                xi,
                                yi + 0.5,
                                f"{yi:.1f}",
        ...
        # ------------------------------------------------------------------
        # Compute and display combined yield/quantity graphs before the tabs
        # ------------------------------------------------------------------
        # Aggregate data across plants to create monthly and weekly combined
        # summaries showing the total good quantity and per‑plant/overall yields.
        monthly_combined, weekly_combined = compute_combined_summaries(filtered_df)
        # Display a section with tabs for monthly and weekly combined charts.
        st.markdown("### 종합 수율 및 양품량")
        combined_tabs = st.tabs(["월간 종합", "주간 종합"])
        with combined_tabs[0]:
            draw_combined_chart(
                monthly_combined,
                f"{selected_plant} 월간 종합 그래프",
                "연-월",
            )
        with combined_tabs[1]:
            draw_combined_chart(
                weekly_combined,
                f"{selected_plant} 주간 종합 그래프",
                "주차",
            )
        ...
        with tab_prod:
            ...
            # Compute yield percentage per process
            prod_df['수율(%)'] = (
                prod_df['양품량'] / prod_df['생산량'] * 100.0
            ).round(2)
            # Sort processes by production quantity for consistent display order
            prod_df = prod_df.sort_values('생산량', ascending=False)
            # First draw the chart before the table.  Use Plotly if available for
            # interactive rendering; otherwise fall back to matplotlib.
            if USE_PLOTLY:
                from plotly.subplots import make_subplots  # type: ignore
                import plotly.graph_objects as go  # type: ignore
                fig = make_subplots(specs=[[{'secondary_y': True}]])
                # Bars for 생산량 and 양품량 on the secondary y-axis (right)
                fig.add_trace(
                    go.Bar(
                        x=prod_df['Process'],
                        y=prod_df['생산량'],
                ...
                # Line for 수율(%) on the primary y-axis (left)
                fig.add_trace(
                    go.Scatter(
                        x=prod_df['Process'],
                        y=prod_df['수율(%)'],
                        ...
                        text=prod_df['수율(%)'].round(2),
                        textposition='top center',
                    ),
                    secondary_y=False,
                )
                # Configure axes: left y-axis for yield, right y-axis for quantity
                fig.update_layout(
                ...
                fig.update_yaxes(title_text='수량', secondary_y=True)
                st.plotly_chart(fig, use_container_width=True)
            else:
                # Fallback to matplotlib: line on primary axis (left) and bars on
                # secondary axis (right)
                fig, ax1 = plt.subplots(figsize=(8, 4))
                processes = prod_df['Process']
                x = np.arange(len(processes))
                # Primary axis for yield
                ax1.set_ylabel('수율 (%)')
                # Plot yield as line with markers
                ax1.plot(x, prod_df['수율(%)'], color='#ee6666', marker='o', label='수율(%)')
                # Annotate yield values
                for xi, yi in enumerate(prod_df['수율(%)']):
                    ax1.text(xi, yi + 0.5, f"{yi:.1f}", ha='center', va='bottom', fontsize=8)
                ax1.set_ylim(50, 100)
                # Secondary axis for quantities
                ax2 = ax1.twinx()
                width = 0.35
                ax2.bar(x - width/2, prod_df['생산량'], width=width, color='#5470c6', label='생산량')
                ax2.bar(x + width/2, prod_df['양품량'], width=width, color='#91cc75', label='양품량')
                ax2.set_ylabel('수량')
                # Annotate bars: annotate each bar group
                for xi in range(len(processes)):
                    ax2.text(
                        x[xi] - width/2,
                        prod_df['생산량'].iloc[xi],
                        f"{int(prod_df['생산량'].iloc[xi]):,}",
                        ha='center', va='bottom', fontsize=8, color='#5470c6'
                    )
                    ax2.text(
                        x[xi] + width/2,
                        prod_df['양품량'].iloc[xi],
                        f"{int(prod_df['양품량'].iloc[xi]):,}",
                ...
                fig.tight_layout()
                st.pyplot(fig)
            # Format numbers and show the table after the chart
