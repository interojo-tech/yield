...
def main() -> None:
    ...
    if st:
        ...
        # Construct a year-week key by concatenating the year and the numeric part
        # of the week.  Extract digits from the '주차' column to ensure keys
        # like '2026-3' are formed consistently even if the original column
        # includes a 'W' prefix.  Weeks without digits are dropped.
        df_week["year_week"] = (
            df_week["년도"].astype(int).astype(str)
            + "-"
            + df_week["주차"].astype(str).str.extract(r"(\d+)")[0]
        )
        df_week = df_week[df_week["year_week"].notna()]
        # Compute weekly summary using the year_week key.  Each unique key
        # corresponds to a separate time bucket (e.g., '2026-2', '2026-3').
        ...
        if not weekly_summary.empty:
            # Reorder weekly summary by extracting numeric week numbers from
            # the existing index and assigning labels of the form 'W#'.  This
            # approach is robust to indices that are strings (e.g. '2026-3'),
            # tuples (e.g. ('2026-3',)) or other formats.  Any non‑numeric
            # week numbers are mapped to zero to ensure they sort first.
            try:
                import re
                week_nums: list[int] = []
                labels: list[str] = []
                for idx_val in weekly_summary.index:
                    key = idx_val
                    # If index is a tuple, take the first element
                    if isinstance(key, tuple) and len(key) > 0:
                        key = key[0]
                    # Extract all integer substrings from the key
                    matches = re.findall(r"(\d+)", str(key))
                    if matches:
                        # Use the last integer found as the week number.  This
                        # captures the week portion of keys like '2026-2' by
                        # selecting '2'.  If the index is simply '2' then it
                        # yields '2' as well.
                        wn = int(matches[-1])
                        week_nums.append(wn)
                        labels.append(f"W{wn}")
                    else:
                        # Use 0 for sorting when no digits are found
                        week_nums.append(0)
                        labels.append(str(key))
                # Create a temporary Series for sorting and get sorted indices
                # Build a mapping from original index to its 'W#' label
                labels_map = {orig_idx: lab for orig_idx, lab in zip(weekly_summary.index, labels)}
                # Create a Series for sorting using the extracted week numbers
                tmp_series = pd.Series(week_nums, index=weekly_summary.index)
                sorted_indices = tmp_series.sort_values().index
                # Reindex weekly_summary according to the sorted indices
                weekly_summary = weekly_summary.loc[sorted_indices]
                # Assign new labels in the sorted order using the mapping
                weekly_summary.index = [labels_map[idx] for idx in sorted_indices]
                weekly_summary.index.name = '주차'
            except Exception:
                # If any error occurs, leave the weekly summary as is
                pass
