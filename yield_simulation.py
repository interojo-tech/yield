...
# Define a colour palette for charts and visual elements.  These
# pastel colours provide a consistent and modern aesthetic across
# interactive and static charts.  The colours were chosen to be
# visually distinct and harmonious when used together.
COLOR_PALETTE = [
    "#4c78a8",  # Blue
    "#f58518",  # Orange
    "#e45756",  # Red
...
# Lightened versions of the above colours for card backgrounds.
CARD_BG_COLORS = [
    "#dbe4ed",  # Light blue
    "#fde6d0",  # Light orange
    "#f9dddd",  # Light red
...
# Apply the custom colour palette to Matplotlib charts by setting
# the global axes colour cycle.  This ensures that all static charts
# (used when Plotly is unavailable) use the same pastel colours as
# interactive charts.  We wrap the assignment in a try/except so that
# the script does not fail on older Matplotlib versions that may not
# support setting the cycler in this manner.
if 'plt' in globals():
    try:
        import matplotlib as _mpl  # local import to avoid exposing as public API
        _mpl.rcParams['axes.prop_cycle'] = _mpl.cycler(color=COLOR_PALETTE)
    except Exception:
        pass
...
def main() -> None:
    ...
    if st:
        ...
        # Determine the cutoff date of the data for display.  Use the last
        # production date in the dataset to indicate when the data was last updated.
        data_cutoff_str = ""
        try:
            _max_date = df["생산일자"].max()
            _dt = pd.to_datetime(_max_date)
            # Format as '26년MM월DD일' where 26 is last two digits of year
            data_cutoff_str = f"{_dt.year % 100:02d}년{_dt.month:02d}월{_dt.day:02d}일"
        except Exception:
            if "생산일자" in df.columns and not df["생산일자"].empty:
                data_cutoff_str = str(df["생산일자"].max())
        ...
        st.markdown(
            ...
            /* Global page background: apply a subtle gradient to the
               entire application to create depth and visual interest. */
            .stApp {
                background: linear-gradient(135deg, #ffffff 0%, #f8f9ff 100%);
            }

            /* Style the Streamlit sidebar: give it a white background,
               rounded corners and a soft shadow.  This makes the
        ...
        # Fix the y-axis range for all yield charts.  We do not
        # display a slider so that the design remains clean and
        # consistent across sessions.  Yields are always plotted
        # between 50% and 100%.
        ...
        for idx, proc in enumerate(process_order):
            with kpi_cols[idx]:
                # Choose a light background colour for each card based on the
                # process index.  CARD_BG_COLORS is defined globally and
                # contains pastel shades corresponding to the main colour palette.
                bg_colour = CARD_BG_COLORS[idx % len(CARD_BG_COLORS)]
                if proc in proc_summary.index:
                    ...
                    st.markdown(
                        f"<div style='background-color:{bg_colour}; border-radius:8px; padding:10px; font-family:\'Noto Sans CJK KR\', sans-serif;'>"
        ...
        with kpi_cols[-1]:
            ...
            # Use the first light card colour for the overall yield card to
            # maintain visual harmony with the process cards.  CARD_BG_COLORS
            # is defined globally.
            overall_bg = CARD_BG_COLORS[0]
            st.markdown(
                f"<div style='background-color:{overall_bg}; border-radius:8px; padding:10px; font-family:\'Noto Sans CJK KR\', sans-serif;'>"
        ...
        def summarise_and_percent(
            ...
            This implementation computes aggregated yields across all plants
            for each time group.  For each group (e.g., a specific day,
            week or month), the yield for each process is calculated as
            the ratio of total good quantity to total production quantity
            across all plants.  The overall yield is then computed as
            the product of these per‑process yields.  Results are
            expressed as percentages and rounded to two decimal places.
            ...
            data : pd.DataFrame
                ...
                grouping (specified in ``group_cols``) should also be
                present.
            group_cols : Tuple[str, ...]
            ...
            pd.DataFrame
                ...
                are reordered so that standard processes appear first
                followed by any additional processes in the input data.
            """
            # Determine the full list of processes present in the data
            # to ensure we compute yields for all of them.  Use sorted
            ...
            # considered first when reordering columns.  Some standard
            ...
            # any additional processes not in the standard list).  Use
            # list concatenation and comprehension to avoid duplicates.
            all_procs = [p for p in standard_procs] + [p for p in processes if p not in standard_procs]

            # Aggregate yields by grouping on the specified columns.  For
            # each group, compute the total good quantity and total
            # production quantity for each process across all plants.
            result_dict: Dict[Tuple, Dict[str, float]] = {}
            # When group_cols is empty (unlikely), treat the entire
            ...
            for key in group_keys:
                ...
                for proc in all_procs:
                    ...
                    # Handle missing or zero production: yield is neutral
                    if prod_sum == 0:
                        yield_val = 1.0
                    else:
                ...
                # Compute overall yield as the product of per‑process
                # yields (expressed as fractions).  Note: if a process
                # yield is 100% (i.e. neutral), it does not affect the
                # product.
                overall = 1.0
                for proc in all_procs:
                    overall *= (yields[proc] / 100.0)
                yields['overall_yield'] = overall * 100.0
            ...
            # If group_keys are tuples (e.g. (year, month)), pandas will
            # use a MultiIndex; otherwise it uses a simple Index.  To
            # match the expected order, do not alter the index here.
            return summary
        ...
        # Convert the index to string dates safely.  Some indices may not be
        # datetime objects (e.g. strings or tuples).  Attempt to convert
        # with pandas.to_datetime; if it fails, fall back to casting to str.
        try:
        ...
        def draw_line_chart(summary: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # Apply our custom colour palette so that each process line
                # uses a visually distinct pastel colour.  The palette is
                # defined globally in COLOR_PALETTE.
                ...
                # Update layout with black font and axis labels
                fig.update_layout(
                ...
                # Set axis tick and label colours to black
        ...
        def draw_product_chart(pivot_df: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # Set y-axis range to 50–100 to match other yield charts and set text colour to black
                fig.update_yaxes(range=[50, 100], color='black')
                # Apply the custom colour palette so that product groups share
                # the same aesthetic as other charts.  Note that if there
                # are more product groups than colours, Plotly will cycle
                # through the palette.
                fig.update_layout(
                ...
                # Show data labels above points and set their colour to black
                fig.update_traces(text=df_long["수율"], textposition="top center", textfont=dict(color='black'))
                # Set x-axis label and tick colour to black
            ...
            if USE_PLOTLY:
                ...
                fig.add_trace(
                    go.Bar(
                        ...
                        y=bar_values,
                        name='양품량',
                        # Use a pastel colour for the bars to match the overall
                        # design.  We choose the first light card colour from
                        # CARD_BG_COLORS.
                        marker_color=CARD_BG_COLORS[0],
                        # Format bar labels without unit suffix (one decimal)
                        text=[f"{v:.1f}" for v in bar_values],
                        textposition='outside',
                        # Set the text colour to the corresponding main colour
                        # so that labels are visible against the light bar.
                        textfont=dict(color=COLOR_PALETTE[0]),
                        opacity=0.8,
                    ),
                    secondary_y=True,
                )
                # Choose darker colours for the yield lines to enhance contrast.  We
                # darken each colour by reducing its luminance.  If colour
                # conversion fails, fall back to the original colour.
                line_colors = []
                for c in COLOR_PALETTE[:4]:
                    try:
                        r = int(c[1:3], 16)
                        g = int(c[3:5], 16)
                        b = int(c[5:7], 16)
                ...
                # Lines for yields (use primary y-axis on the left).  Increase
                # line width and marker size for better visibility.  Use bold
                # font for 종합수율 data labels.
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            textfont=dict(
                                ...
                                size=12,
                                family="sans-serif",
                                weight="bold" if col == '종합수율' else "normal",
                            ),
                        ),
                        secondary_y=False,
                    )
                # Update layout: adjust legend position, add axis titles and apply
                # black font colour throughout.
                fig.update_layout(
                    title=title,
                    xaxis_title=xlabel,
                    bargap=0.2,
                ...
                # Left y-axis for yield (%) with fixed range 60–90
                fig.update_yaxes(title_text='수율 (%)', secondary_y=False, range=[60, 90], color='black')
                # Right y-axis for quantity (in millions).  Set axis label colour to black
                fig.update_yaxes(title_text='수량 (M)', secondary_y=True, color='black')
                # Ensure x-axis tick labels are black
                fig.update_xaxes(color='black')
                st.plotly_chart(fig, use_container_width=True)
            else:
                ...
                # Use a pastel colour for the bars to match the design.  The
                # first light card colour (CARD_BG_COLORS[0]) is used here.
                bars = ax_qty.bar(x, bar_vals, color=CARD_BG_COLORS[0], label='양품량', alpha=0.8, zorder=1)
                ...
                for i, b in enumerate(bars):
                    ...
                    ax_qty.annotate(
                        ...
                        va='bottom',
                        fontsize=8,
                        # Use the corresponding main colour for the text
                        # so that it stands out against the light bar.
                        color=COLOR_PALETTE[0],
                        zorder=2,
                    )
                # Darken line colours for better contrast and plot yield lines
                line_colors = []
                for c in COLOR_PALETTE[:4]:
                    try:
                        ...
                        line_colors.append(c)
                # Plot yields with thicker lines and larger markers; bold labels for 종합수율
                for col, color in zip(['A관', 'C관', 'S관', '종합수율'], line_colors):
                ...
                # Colour ticks and axis for both axes
        ...
        # ------------------------------------------------------------------
        st.subheader(f"종합 수율 및 양품량 (Data 기준일: {data_cutoff_str})")
        ...
        with tab_prod:
            ...
            if USE_PLOTLY:
                ...
                # Set axis colours and ranges.  Primary y-axis shows quantity; secondary shows yield.
                fig.update_yaxes(title_text='수량', secondary_y=False, color='black')
                # Set the yield axis to range between 50 and 100 and colour to black to match other yield charts
