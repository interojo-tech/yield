...
def main() -> None:
    ...
    if st:
        ...
        with tab_prod:
            ...
            # Aggregate production and good quantity by process
            prod_df = (
            ...
            # Bar chart of production quantity by process
            if USE_PLOTLY:
                fig = px.bar(
                    prod_df,
                    x="Process",
                    y="생산량",
                    title=f"{selected_plant} 공정별 생산량",
                    labels={"Process": "공정", "생산량": "생산량"},
                )
                fig.update_layout(xaxis_title="공정", yaxis_title="생산량")
                st.plotly_chart(fig, use_container_width=True)
            else:
                fig, ax = plt.subplots(figsize=(8, 4))
                ax.bar(prod_df["Process"], prod_df["생산량"])
                ax.set_title(f"{selected_plant} 공정별 생산량")
                ax.set_xlabel("공정")
                ax.set_ylabel("생산량")
                ax.tick_params(axis="x", rotation=45)
                fig.tight_layout()
