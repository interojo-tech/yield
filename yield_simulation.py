...
def main() -> None:
    ...
    if st:
        ...
        def draw_line_chart(summary: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            numeric_vals = summary.select_dtypes(include=["number"]).to_numpy().flatten()
            numeric_vals = numeric_vals[~pd.isna(numeric_vals)]
            if len(numeric_vals) == 0:
                y_min_default, y_max_default = 0.0, 100.0
            else:
                y_min_default = float(max(0.0, np.floor(numeric_vals.min() - 5)))
                y_max_default = float(min(100.0, np.ceil(numeric_vals.max() + 5)))
                if y_max_default <= y_min_default:
                    y_max_default = min(100.0, y_min_default + 10.0)

            slider_key = "yaxis_" + "".join(ch if ch.isalnum() else "_" for ch in title)
            y_range = st.slider(
                "Y축 범위 조절",
                min_value=0.0,
                max_value=100.0,
