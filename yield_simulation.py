...
def main() -> None:
    ...
    if st:
        ...
        # Determine available week options as integers, then convert to strings.  We
        # exclude week numbers less than 2 so that the first week shown is W2.
        # Gather available week numbers as integers, filter out week 1, and sort ascending.
        # These numbers will later be displayed with a 'W' prefix so that week 2
        # appears as 'W2', week 3 as 'W3', etc.  Sorting ensures W10 and W11
        # appear after W9, and any future weeks (W12, W13, ...) append at the end.
        weeks_int = sorted(df["주차"].dropna().astype(int).unique().tolist())
        weeks_int = [w for w in weeks_int if w >= 2]
        # Convert to strings with 'W' prefix for display in the week selector
        weeks = [f"W{w}" for w in weeks_int]
        ...
        if selected_week != "전체":
            # Extract numeric week from a label like 'W2' -> 2, then filter accordingly
            try:
                week_num = int(str(selected_week).lstrip('W'))
            except Exception:
                week_num = None
            if week_num is not None:
                filtered_df = filtered_df[filtered_df["주차"].astype(int) == week_num]
        ...
        # ------------------------------------------------------------------
        # Aggregate production and good quantity by process and compute individual yields
        proc_summary = (
        ...
        # Define the order of processes explicitly before computing the overall yield.
        process_order = [
            "사출조립",
            "분리",
            "하이드레이션/전면검사",
        ...
        # Compute overall yield as the product of process‑specific yields.  Missing
        # processes are treated as yield 1.0 so they do not affect the product.
        overall_yield = 1.0
        for proc in process_order:
            if proc in proc_summary.index:
                ...
                overall_yield *= 1.0
        # Setup KPI panels: one column per process plus one for the overall yield
        ...
        # Reindex weekly summary so that only weeks >= 2 are included and the
        # index labels are user‑friendly (e.g., 'W2', 'W3', ...).  Sorting by the
        # underlying numeric week ensures W10, W11, etc. appear after W9 and
        # future weeks append naturally.
        if not weekly_summary.empty:
            try:
                # Extract numeric week number from the composite year_week index.
                week_nums = weekly_summary.index.to_series().str.extract(r"-(\d+)")[0].astype(int)
                # Filter out week numbers less than 2 (exclude W1)
                valid_mask = week_nums >= 2
                weekly_summary = weekly_summary.loc[valid_mask.values]
                week_nums = week_nums[valid_mask]
                # Sort by numeric week
                sorted_idx = week_nums.sort_values().index
                weekly_summary = weekly_summary.loc[sorted_idx]
                # Create new index labels prefixed with 'W'
                new_labels = [f"W{n}" for n in week_nums.loc[sorted_idx].values]
        ...
        with tab_daily:
            ...
            # Draw the full daily summary chart first.  The chart shows all dates and
            # processes.  Users can later filter the table without affecting the chart.
        ...
        with tab_weekly:
            ...
            # Draw the weekly summary chart before the search box.  This chart
            # includes all available weeks and processes.
