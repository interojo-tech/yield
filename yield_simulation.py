...
def main() -> None:
    ...
    if st:
        ...
        overall_yield = 1.0
        # Use the globally defined process order when computing the overall yield.  We
        # copy the list here so that modifications (e.g., reordering) in this
        # context do not affect the global constant.  By computing the product of
        # yields in this specific order we ensure the overall yield reflects the
        # multiplicative combination requested by the user.  Missing processes
        # default to a yield of 1.0.
        ...
        for proc in process_order:
            if proc in proc_summary.index:
                ...
                overall_yield *= 1.0
        # process_order is already defined above; do not redefine it here.
        ...
        # Convert the MultiIndex (년도, 월) into a single string index of the form
        # 'YYYY-<월>'.  In some cases the index may have been collapsed to a
        # single level (e.g., if summarise_and_percent averaged duplicates);
        # therefore we handle both MultiIndex and simple Index cases.  If
        # MultiIndex is present, use to_frame to extract columns and build
        # the combined label.  Otherwise, cast the existing index to string.
        try:
            if isinstance(monthly_summary.index, pd.MultiIndex):
                idx_df = monthly_summary.index.to_frame(index=False)
                monthly_summary.index = idx_df.apply(
                    lambda r: f"{int(r['년도'])}-{r['월']}", axis=1
                )
            else:
                monthly_summary.index = monthly_summary.index.astype(str)
        except Exception:
            # As a fallback, leave the index unchanged but cast to string
            monthly_summary.index = monthly_summary.index.astype(str)
        ...
        with tab_daily:
            ...
            # Draw the daily summary chart using only the most recent 30 days.  The
            # summary DataFrame is sorted chronologically, so taking the last 30
            # rows will correspond to the latest 30 days.  If fewer than 30 days
            # are available, use all available data.  This ensures the chart
            # remains focused on recent trends.
        ...
        with tab_weekly:
            ...
            # Draw the weekly summary chart using only the most recent 10 weeks.
            if not weekly_summary.empty:
