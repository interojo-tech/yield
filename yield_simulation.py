...
def main() -> None:
    ...
    if st:
        ...
        # ------------------------------------------------------------------
        # Product-level yield summary and full dataset toggle
        # ------------------------------------------------------------------
        # Compute product summary using the '신규분류요약' column rather than '품명'.
        # This groups records by defect classification and summarises the
        # production quantity, good quantity and yield percentage.
        prod_group = "신규분류요약"
        product_summary_df = (
        ...
        st.subheader("제품군별 수율 현황 (신규분류요약 기준)")
        ...
        # to remember the toggle state across reruns.  Display a single
        # button whose label changes depending on whether the dataset is
        # currently visible.  When the button is clicked, immediately
        # update the state so that the dataset appears or disappears
        # and the button label flips accordingly.
        if 'show_data' not in st.session_state:
            st.session_state['show_data'] = False
        if st.session_state['show_data']:
            # Dataset is currently visible; provide hide button
            # Show the dataset
            ...
            if st.button("전체 데이터 숨기기"):
                st.session_state['show_data'] = False
        else:
            # Dataset is currently hidden; provide show button
