...
if 'plt' in globals():
    # Select a font that supports Hangul characters.  We try Noto Sans CJK KR
    # first because it is commonly included on many Linux distributions
    # (including this environment).  If it cannot be found, we fall back
    # to DejaVu Sans or the default sans‑serif font.  We always disable
    # unicode_minus to ensure minus signs render correctly.
    try:
        plt.rcParams['font.family'] = 'Noto Sans CJK KR'
        plt.rcParams['axes.unicode_minus'] = False
    except Exception:
        # If Noto Sans CJK KR is not available, try other fonts.
        try:
            plt.rcParams['font.family'] = 'NanumGothic'
            plt.rcParams['axes.unicode_minus'] = False
        except Exception:
            # Final fallback to DejaVu Sans (supports many Unicode blocks but
            # may not include Hangul).  The end user may customise this.
            try:
...
def main() -> None:
    ...
    if st:
        ...
        # ------------------------------------------------------------------
        # Display the filtered raw dataset at the bottom
        # ------------------------------------------------------------------
