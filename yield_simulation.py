...
# Define a module‑level constant for the desired process order.  Defining this
# list outside of the main() function ensures it is available in any
# execution path and avoids UnboundLocalError issues where a locally scoped
# variable might be referenced before assignment.  The order here controls
# the display order of processes in tables and charts and is used when
# calculating the compounded "overall" yield.  If additional processes
# appear in the dataset they will be appended after these five.
PROCESS_ORDER = [
    "사출조립",
    "분리",
    "하이드레이션/전면검사",
...
def main() -> None:
    ...
    if st:
        ...
        # Define the desired order of processes for display and overall yield calculation.
        # Use the module‑level constant so that the order is consistent across the
        # application and to avoid an UnboundLocalError from referencing
        # process_order before assignment.  Assigning to a local variable here
        # allows the remainder of the logic to use this name without affecting
        # the global constant.
        ...
        def summarise_and_percent(
            ...
            # PROCESS_ORDER list and append the overall yield column name.  Any
            # additional process columns in the DataFrame will be appended
            # afterwards.  This prevents mis‑ordering when new processes are
