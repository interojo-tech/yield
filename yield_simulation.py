...
def main() -> None:
    ...
    if st:
        ...
        # ------------------------------------------------------------------
        # Compute product (품명) summary before the full dataset.  This table
        # summarises production quantity, good quantity and yield percentage
        # by product name.  It is displayed above the full dataset toggle.
        product_summary_df = (
        ...
        product_summary_df = product_summary_df.reset_index().sort_values(
            "품명"
        )
        ...
        button_label = '전체 데이터 보기' if not st.session_state['show_data'] else '전체 데이터 숨기기'
