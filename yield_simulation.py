    ...
    if st:
        ...
        def compute_combined_summaries(full_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
            ...
            if not weekly_combined.empty:
                # Sort by year and week, then preserve the year in the display label
                # so recent-week slicing uses the latest 2026 data rather than older 2025 weeks.
                weekly_combined = weekly_combined.sort_values(['년도', '주차번호'])
                weekly_combined['주차'] = weekly_combined.apply(
                    lambda row: f"{int(pd.to_numeric(row['년도'], errors='coerce'))}-W{int(row['주차번호']):02d}",
                    axis=1,
                )
        ...
        with comb_cols[1]:
            ...
            draw_combined_chart(_sort_limit_weekly_df(weekly_combined, 10), "주간 양품량 및 수율", "주차", (65.0, 85.0))
