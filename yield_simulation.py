...
def main() -> None:
    ...
    if st:
        ...
        with tab_daily:
            ...
            daily_df = daily_summary.reset_index().rename(columns={"index": "날짜"})
            ...
            # Provide a dropdown to search by production date within the filtered dataset.  When
            # a specific date is selected, display only the records from that date.  Use a
            # unique key for the selectbox to avoid clashes across tabs.
            ...
            if date_option != '전체':
                df_date = filtered_df[
                    filtered_df['생산일자'].dt.strftime('%Y-%m-%d') == date_option
                ].copy()
                df_date['수율(%)'] = (df_date['yield'] * 100).round(2)
                df_date_fmt = format_numbers(df_date)
                st.subheader(f"{selected_plant} - {date_option} 데이터")
                st.dataframe(df_date_fmt, use_container_width=True)
        with tab_weekly:
            st.subheader(f"{selected_plant} - 주간 수율")
            weekly_df = weekly_summary.reset_index().rename(columns={"index": "주차"})
            ...
            # Production date search for weekly tab
            unique_dates = sorted(filtered_df['생산일자'].dt.strftime('%Y-%m-%d').unique())
            date_option_w = st.selectbox(
                '생산일자 검색',
                options=['전체'] + unique_dates,
                index=0,
            ...
            if date_option_w != '전체':
                df_date_w = filtered_df[
                    filtered_df['생산일자'].dt.strftime('%Y-%m-%d') == date_option_w
                ].copy()
                df_date_w['수율(%)'] = (df_date_w['yield'] * 100).round(2)
                df_date_w_fmt = format_numbers(df_date_w)
                st.subheader(f"{selected_plant} - {date_option_w} 데이터")
                st.dataframe(df_date_w_fmt, use_container_width=True)
        with tab_monthly:
            st.subheader(f"{selected_plant} - 월별 수율")
            monthly_df = monthly_summary.reset_index().rename(columns={"index": "연-월"})
            ...
            # Production date search for monthly tab
            unique_dates = sorted(filtered_df['생산일자'].dt.strftime('%Y-%m-%d').unique())
            date_option_m = st.selectbox(
                '생산일자 검색',
                options=['전체'] + unique_dates,
                index=0,
            ...
            if date_option_m != '전체':
                df_date_m = filtered_df[
                    filtered_df['생산일자'].dt.strftime('%Y-%m-%d') == date_option_m
                ].copy()
                df_date_m['수율(%)'] = (df_date_m['yield'] * 100).round(2)
                df_date_m_fmt = format_numbers(df_date_m)
                st.subheader(f"{selected_plant} - {date_option_m} 데이터")
                st.dataframe(df_date_m_fmt, use_container_width=True)
        with tab_prod:
