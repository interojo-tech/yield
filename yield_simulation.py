...
def main() -> None:
    ...
    if st:
        ...
        # Include the file modification time in the cache key so that the cache
        # automatically refreshes whenever the Excel file is updated.
        ...
        # Determine the last modified timestamp of the Excel file. If retrieval
        # fails, use zero so that Streamlit can still compute a cache key.
        try:
            _file_mtime = os.path.getmtime(file_path)
        except Exception:
            _file_mtime = 0.0

        df = load_cached(file_path, _file_mtime)
        # 안전장치: 캐시에서 반환된 객체가 DataFrame이 아닐 경우 직접 로드합니다.
