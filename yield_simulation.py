...
def main() -> None:
    ...
    if st:
        ...
        st.markdown(
            ...
            /* Previously the first child of the radio group contained an empty label,
               resulting in a blank space below the header.  Because we now pass
               an empty string as the radio label (see `st.radio()` call), the
               label wrapper no longer renders and this rule is unnecessary.
               Leaving it empty preserves backwards compatibility without
        ...
        if not weekly_summary.empty:
            try:
                # Extract numeric week part from each index entry (e.g., '2026-12')
                # and convert to integer. Use original index to maintain order
                # when duplicates occur. sort_values will keep relative order of
                # duplicates stable.
                week_nums_series = weekly_summary.index.to_series().str.extract(r"-(\d+)", expand=False)[0].astype(int)
                sorted_idx = week_nums_series.sort_values(kind="stable").index
                weekly_summary = weekly_summary.loc[sorted_idx]
                new_labels = [f"W{n}" for n in week_nums_series.loc[sorted_idx].values]
