...
# ----------------------------------------------------------------------
# Data loading and preprocessing for interactive dashboard
# ----------------------------------------------------------------------

def _load_and_prepare(path: str) -> pd.DataFrame:
    """Load and prepare production data for analysis.

    This helper reads the workbook at ``path``, normalises process names,
    calculates per-record yield, and derives year/month/week columns.
    It also converts week identifiers (e.g., 'W3') to integer week numbers.
    ...
    -------
    pd.DataFrame
        Preprocessed DataFrame ready for aggregation and visualisation.
    """
    df_local = load_data(path)
    # Normalise process names and compute per-record yield
    df_local = map_process_codes(df_local)
    df_local = compute_yields(df_local)
    # Derive or clean date-related columns
    # 년도: integer year extracted from 생산일자
    if "년도" not in df_local.columns or df_local["년도"].isna().all():
        df_local["년도"] = df_local["생산일자"].dt.year
    # 월: month as string with '월' suffix (e.g. '1월', '2월')
    if "월" not in df_local.columns or df_local["월"].isna().all():
        df_local["월"] = df_local["생산일자"].dt.month.astype(str) + "월"
    # 주차: ensure numeric week number.  The source data may contain strings
    # such as 'W3' or may already be numeric.  We extract digits where
    # possible and fall back to ISO week numbers derived from 생산일자.
    # Compute ISO week number; .dt.isocalendar().week returns a pandas
    # Series with dtype Int64 which can include NA.  Fill missing values
    # with 0 to allow safe conversion to int.
    iso_week = df_local["생산일자"].dt.isocalendar().week
    # Replace <NA> with 0 and convert to integer type
    iso_week_filled = iso_week.fillna(0).astype(int)
    if "주차" in df_local.columns:
        # Extract numeric digits from existing 주차 values; coerce to float
        extracted = df_local["주차"].astype(str).str.extract(r"(\d+)")[0]
        extracted_num = pd.to_numeric(extracted, errors="coerce")
        # Combine extracted numbers with ISO week numbers; when extracted is
        # missing, use iso_week_filled.  Remaining NAs (if any) will be 0.
        week_numeric = extracted_num.fillna(iso_week_filled)
    else:
        week_numeric = iso_week_filled
    # Convert to integer, replacing any remaining NaNs with 0
    week_numeric = pd.to_numeric(week_numeric, errors="coerce").fillna(0).astype(int)
    df_local["주차"] = week_numeric
...
def main() -> None:
    ...
    if st:
        ...
        # Use numeric week numbers for sorting; convert to int
        week_nums = sorted(df["주차"].dropna().astype(int).unique().tolist())
        processes = sorted(df["Process"].dropna().unique().tolist())
        year_options = ["전체"] + [str(y) for y in years]
        month_options = ["전체"] + months
        # Display week options with 'W' prefix for clarity
        week_options = ["전체"] + [f"W{w}" for w in week_nums]
        ...
        if selected_week != "전체":
            # Extract numeric week from selection (e.g. 'W3' -> 3) and filter
            try:
                week_num = int(str(selected_week).lstrip("W"))
                filtered_df = filtered_df[filtered_df["주차"] == week_num]
            except Exception:
                pass
        ...
        # Filter out rows without a valid week number to avoid conversion errors
        df_week_valid = df_week[df_week["주차"].notna()].copy()
        # Convert week numbers to integers for sorting
        df_week_valid["주차"] = df_week_valid["주차"].astype(int)
        weekly_summary = summarise_and_percent(df_week_valid, group_cols=("주차",))
        ...
        def draw_line_chart(summary: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                tmp = tmp.reset_index()
                # If overall_yield is present, rename it to 종합수율 for clarity
                if "overall_yield" in tmp.columns:
                    tmp = tmp.rename(columns={"overall_yield": "종합수율"})
                # Determine the process columns (all columns except the index)
                process_cols = [c for c in tmp.columns if c != idx_name]
                # Compute minimum, maximum and average yields for each process
                stats_df = tmp[process_cols].agg(["min", "max", "mean"]).transpose()
                stats_df = stats_df.reset_index().rename(
                    columns={"index": "공정", "min": "최소", "max": "최대", "mean": "평균"}
                )
                # Melt to long format
                df_long = tmp.melt(id_vars=[idx_name], var_name="공정", value_name="수율")
                # Attach min, max, avg statistics to each row based on process
                df_long = df_long.merge(stats_df, on="공정", how="left")
                # Create an interactive line chart with markers and custom hover data
                fig = px.line(
                    df_long,
                    x=idx_name,
                    y="수율",
                    ...
                    hover_data={"최소": ":.2f", "최대": ":.2f", "평균": ":.2f"},
                )
                # Set y-axis range to start at zero and adjust automatically
