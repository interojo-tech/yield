    ...
    if st:
        ...
        def summarise_and_percent(
            ...
            try:
                ...
                pass
            # If multiple plants share the same grouping key (e.g. when viewing
            # "전체 공장"), summarise_by_group will produce one row per
        ...
        # Convert the production date index to string (YYYY-MM-DD)
        daily_summary.index = daily_summary.index.strftime("%Y-%m-%d")
        # Sort by date ascending and restrict to the most recent 30 days for the daily graph.
        # Use .iloc to take the last 30 rows, preserving chronological order.  If there
        # are fewer than 30 records the entire summary is used.
        try:
            daily_summary_sorted = daily_summary.sort_index()
            daily_summary_plot = daily_summary_sorted.iloc[-30:]
        except Exception:
        ...
        if not weekly_summary.empty:
            try:
                ...
                # If any error occurs during reordering, leave weekly_summary unchanged.
                pass

        # After reordering weekly summary, retain only the last 10 weeks for the graph.
        try:
        ...
        with tab_daily:
            ...
            # Draw the daily summary chart using only the most recent 30 days.  This
            # plot shows the last 30 dates and processes.  Users can later filter
            # the table without affecting the chart.
        ...
        with tab_weekly:
            ...
            # Draw the weekly summary chart before the search box.  Limit to the
            # most recent 10 weeks to prevent overcrowding and highlight recent
            # trends.
            ...
            if selected_date_weekly == '전체':
                ...
                # Weekly summary indices are labelled as 'W#' (e.g., 'W2', 'W10'),
                # so construct a week label accordingly.  This ensures that
                # selecting a date filters to the correct week number in the
                # summary index without including the year component.
