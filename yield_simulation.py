...
def main() -> None:
    ...
    if st:
        ...
        # ------------------------------------------------------------------
        # Custom date and week range selectors for yield views
        #
        # We move the date range filters above the defect section so that they
        # belong to the yield filter area rather than the defect filter area.
        # Compute a temporary filtered dataset based on the plant, year, month,
        # week and process selections.  This dataset is used only to derive
        # available date and week ranges for the selectors below.
        try:
            tmp_df = df.copy()
            ...
            if selected_year != "전체":
                tmp_df = tmp_df[
                    tmp_df["년도"].astype(int).astype(str) == selected_year
                ]
            ...
            if selected_process != "전체":
                tmp_df = tmp_df[tmp_df["Process"] == selected_process]
        ...
        # can understand which calendar month a particular week belongs to.
        week_entries = []
        try:
        ...
        if week_entries:
            default_week_range = ("선택없음", week_entries[-1])
        else:
            default_week_range = ("선택없음", "선택없음")
        # Weekly range selector: allows selecting a start and end week label.  When
        # both ends are '선택없음' the full range will be used.
        selected_week_range = st.sidebar.select_slider(
            "주간 기간 선택",
            options=week_options_range,
            value=default_week_range,
        )

        # Daily date range picker.  An empty interval (None) indicates that the
        # full available date range should be used.  Users can also select a
        # single date or a start/end date tuple.
        date_range_daily = st.sidebar.date_input(
            "일별 기간 선택",
            value=(_tmp_min_dt, _tmp_max_dt)
        ...
        # week range filtering will be applied later based on selected_week_range.
        ...
        # selected_week_range variable comes from the weekly slider defined in
        # the sidebar.  Extract numeric week numbers from the labels and
        # retain only rows whose week numbers fall within the selected range.
        try:
            start_label, end_label = selected_week_range
            import re as _re
