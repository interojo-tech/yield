...
# Global process order constant.  This list defines the preferred order in
# which processes appear in tables and charts.  It is used throughout the
# dashboard for both display ordering and when computing the compounded
# overall yield as the product of process-specific yields.
PROCESS_ORDER = [
    "사출조립",
    "분리",
    "하이드레이션/전면검사",
...
def main() -> None:
    ...
    if st:
        ...
        # Define the desired order of processes for display and overall yield
        # calculation.  By assigning this list before using it in the loop,
        # we avoid any UnboundLocalError that would occur if the variable were
        # referenced before assignment.  Should new processes appear in the
        # dataset they will be appended after these five.
        process_order = [
            "사출조립",
            "분리",
            "하이드레이션/전면검사",
        ...
        def summarise_and_percent(
            ...
            # If multiple plants contribute to the same grouping key (e.g. when
            # viewing "전체 공장"), summarise_by_group produces one row per
            # plant.  After dropping the plant level this can lead to
            # duplicate indices.  Aggregate duplicated indices by taking the
            # mean across plants to produce a single row per grouping key.
            if summary.index.duplicated().any():
                summary = summary.groupby(level=0).mean()

            # Convert yields to percentage and round for display
            summary = summary * 100.0
            summary = summary.round(2)

            # Determine the column order.  Start with the globally defined
            # PROCESS_ORDER list and append the overall yield column name.
            # Any additional process columns are appended afterwards.  This
            # ensures that new processes appear at the end of the table and
            # that the main processes appear in a consistent order.
            desired_order = PROCESS_ORDER + ['overall_yield']
        ...
        # Compute daily summary.  Each row represents yields for a production date.
        daily_summary = summarise_and_percent(
            filtered_df, group_cols=("생산일자",)
        )
        # Convert the index from Timestamp to string for easier display.
        daily_summary.index = daily_summary.index.strftime("%Y-%m-%d")
        # Sort by date ascending and take only the most recent 30 days for plotting.
        try:
            daily_summary_plot = daily_summary.sort_index().iloc[-30:]
        except Exception:
        ...
        # After reordering weekly summary, keep only the most recent 10 weeks for plotting.
        try:
            weekly_summary_plot = weekly_summary.iloc[-10:]
        except Exception:
        ...
        with tab_weekly:
            ...
            if selected_date_weekly == '전체':
                ...
                # Weekly summary indices use labels like 'W2', 'W3', etc.  Compute
                # the label from the selected date's ISO week number.
                week_label = f"W{week_no}"
                if week_label in weekly_summary.index:
                    summary_weekly_filtered = weekly_summary.loc[[week_label]]
                else:
