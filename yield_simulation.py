...
def main() -> None:
    ...
    if st:
        ...
        with tab_weekly:
            ...
            if not weekly_summary.empty:
                weekly_limit = 10 if selected_plant == "전체 공장" else 12
                weekly_summary_plot = _sort_limit_weekly_df(weekly_summary, weekly_limit)
            else:
            ...
            if selected_date_weekly == '전체':
                ...
                iso_cal = dt_sel.isocalendar()
                week_no = int(iso_cal.week)
                week_year = int(iso_cal.year)
                week_key = f"{week_year}-W{week_no:02d}"
