...
def summarise_by_group(
    ...
    # ------------------------------------------------------------------
    # Remove any mould printing columns.
    #
    # Even though rows containing the substring "몰드" are dropped during
    # initial data loading, downstream summarisation may still produce
    # columns for mould printing if there are stray process names or
    # legacy columns in the pivot table.  To ensure that all mould
    # printing processes (e.g. "몰드인쇄", "몰드인자") are excluded from
    # yield tables and charts, drop any columns whose name contains the
    # substring "몰드".  This is performed after filling missing values
    # so that the multiplicative yield calculation remains well-defined.
    try:
        mold_cols = [c for c in pivot.columns if isinstance(c, str) and "몰드" in c]
        if mold_cols:
            pivot = pivot.drop(columns=mold_cols)
    except Exception:
        # If any unexpected error occurs during column dropping,
        # continue without removal.  The mould rows have already been
        # removed at load time, so this is a safeguard rather than a
        # primary filter.
        pass
...
def main() -> None:
    ...
    # compatibility.
    # List of preferred Excel filenames.  Newer or more specific files
    # should appear earlier in the list so they take precedence when
    # multiple versions exist.  When none of the known names are
    # present, fall back to scanning the current directory for any
    # workbook whose name begins with '공정기술팀 대시보드' and ends with
    # '.xlsx'.  This ensures newly uploaded monthly workbooks (e.g.,
    # '공정기술팀 대시보드(수율).xlsx' with updated weeks) are picked up
    # automatically without requiring code changes.
    # ----------------------------------------------------------------------
    # Determine which Excel workbook to load.  Instead of relying on a
    # predefined list of candidate filenames, scan the current directory for
    # any workbook that looks like a yield report.  Prefer files whose
    # names contain "수율" (indicating yield data) and choose the most
    # recently modified workbook.  This ensures that updated files (even
    # when names remain unchanged) are picked up automatically and that
    # newly added weeks (e.g. W12, W13, etc.) are reflected without
    # manual filename updates.
    file_path: str | None = None
    dirname = os.path.dirname(__file__)
    # Collect all Excel files that start with the expected prefix and end with .xlsx
    all_candidates: list[str] = []
    for fname in os.listdir(dirname):
        if fname.startswith("공정기술팀") and fname.endswith(".xlsx"):
            all_candidates.append(fname)
    if all_candidates:
        # Split into yield-specific and other files
        yield_candidates = [f for f in all_candidates if "수율" in f]
        search_list = yield_candidates or all_candidates
        # Sort by modification time (most recent first)
        search_list.sort(key=lambda f: os.path.getmtime(os.path.join(dirname, f)), reverse=True)
        file_path = os.path.join(dirname, search_list[0])
    if file_path is None:
        print(
            "Error: No Excel data file was found. Expected a file starting with "
            "'공정기술팀' and ending with '.xlsx' in the same directory as this script."
        )
        return
    ...
    def _load_and_prepare(path: str) -> pd.DataFrame:
        ...
        # Exclude all records related to mould printing.  The source data may
        # contain mould references in several columns (e.g. 공정코드, Process,
        # 신규분류요약) and in slightly different formats.  To ensure these
        # rows are removed from yield analysis, build a boolean mask across
        # the relevant fields and drop any row where the substring "몰드"
        # appears.  This covers variants such as "몰드인쇄" and
        # "몰드인자" even if the mapping step has not normalised
        # whitespace or bracket codes.
        mold_mask = pd.Series(False, index=df_local.index)
        # Check common fields for the substring '몰드'.  Use astype(str) and
        # na=False to handle missing values gracefully.
        for col in [
            "공정코드",
            "Process",
            "신규분류요약",
            "품명" if "품명" in df_local.columns else None,
        ]:
            if col and col in df_local.columns:
                mold_mask |= df_local[col].astype(str).str.contains("몰드", na=False)
        if mold_mask.any():
            df_local = df_local[~mold_mask]
    ...
    if st:
        ...
        # Sidebar selections
        # ------------------------------------------------------------------
        # Sidebar section for yield analysis.  Provide plant, year, month,
        # week and process selectors.  Use the original naming convention
        # for the plant selector ("전체" rather than "전체 공장") to match
        # the behaviour before the recent design changes.  We do not
        # prefix the list with any separate heading so that the layout
        # remains uncluttered.
        # Build plant list, dropping NaN values before sorting.  Sorting directly on
        # a list containing NaN/None alongside strings raises a TypeError in
        # Python 3 because these types cannot be compared.  By dropping
        # missing values and converting to strings, we avoid such errors.
        plant_values = df["공장"].dropna().unique().tolist()
        plant_values = [str(p) for p in plant_values]
        plants = ["전체"] + sorted(plant_values)
        selected_plant = st.sidebar.selectbox(
            "공장 선택", options=plants, index=0
        )
        # Year, month and week selections
        years = sorted(df["년도"].dropna().astype(int).unique().tolist())
        months = sorted(df["월"].dropna().astype(str).unique().tolist())
        # Determine available week options.  Extract all unique week numbers
        # present in the data (including double‑digit weeks) and convert
        # them to strings prefixed with 'W'.  This allows the week
        # selector to grow automatically as new weeks (e.g., W12, W13) are
        # added to the dataset.  We avoid filtering out week 1 here so
        # that the full range of data is available.
        # Determine available week options. Extract the numeric part of the week
        # from the '주차' column for each record. Handles values like 'W12',
        # '12', or numeric types. Keep all week numbers present in the data
        # without limiting to a fixed number of weeks; this ensures newly added
        # weeks (e.g., W12, W13, etc.) automatically appear in the selector.
        # Build week options from all distinct numeric week values present in the dataset.
        # Extract digits from the '주차' column (handles values like 'W12', '12', etc.),
        # convert to integers and sort.  This ensures that if the underlying data
        # contains weeks beyond W11 (e.g. W12, W13, etc.), those weeks will
        # automatically appear in the week selector and the corresponding charts.
        weeks: List[str] = []
        if "주차" in df.columns:
            try:
                # Extract numeric portions from the week column.  When
                # expand=False, str.extract returns a Series of matched
                # substrings (or NaN).  Do not index [0] on the result, as
                # doing so incorrectly selects the first element of the
                # Series rather than returning the full Series.  Convert
                # the extracted strings to numeric values, coerce errors
                # to NaN, and drop missing entries.  This yields all
                # unique week numbers present in the data, including
                # double‑digit weeks like 12 and 13.
                week_str = df["주차"].astype(str).str.extract(r"(\d+)", expand=False)
                week_nums = pd.to_numeric(week_str, errors="coerce").dropna().astype(int)
                unique_week_nums = sorted(set(week_nums.tolist()))
                weeks = [f"W{w}" for w in unique_week_nums]
            except Exception:
                weeks = []
        # Use the global process order rather than sorting alphabetically.  This ensures
        # the process selection appears in the order 사출조립 → 분리 → 하이드레이션/전면검사 → 접착/멸균 → 누수/규격검사.
        processes = [p for p in PROCESS_ORDER if p in df["Process"].dropna().unique().tolist()]
        ...
        # Use the same naming convention as the yield plant selection.  Provide
        # "전체 공장" followed by each plant for the defect section as well.
        defect_plant_values = df["공장"].dropna().unique().tolist()
        defect_plant_values = [str(p) for p in defect_plant_values]
        defect_plants = ["전체 공장"] + sorted(defect_plant_values)
        ...
        selected_defect_plant = st.sidebar.selectbox("공장 선택 (불량)", options=defect_plants, index=0)
        selected_defect_process = st.sidebar.selectbox(
            "공정 선택 (불량)", options=defect_processes, index=0, key="defect_process_select"
        )
        selected_defect_type = st.sidebar.selectbox(
            "불량 유형 선택", options=defect_types, index=0, key="defect_type_select"
        )
        # Save the defect selections to session state so the defect dashboard can
        # access them.  By persisting these values in session_state, the
        # `run_defect_dashboard` function can retrieve them and filter its
        # charts accordingly.
        st.session_state['defect_plant'] = selected_defect_plant
        st.session_state['defect_process'] = selected_defect_process
        st.session_state['defect_type'] = selected_defect_type
        ...
        # Header and title
        # ------------------------------------------------------------------
        # Inject custom CSS to style the dashboard selector and headers.  We avoid
        # wrapping the radio buttons in a single large box so that each option
        # appears as its own bordered button.  The class names target the
        # Streamlit radio group elements.  Header sizes are adjusted per
        # user feedback: the main header (company name) is slightly smaller
        # than before, while the dashboard title remains prominent.
        st.markdown(
            """
            <style>
            /* Style each radio option as an individual button with border and larger font. */
            ...
            /* Remove any border, margin or background from the radio group container itself */
            div[data-testid="stRadio"] {
                ...
                /* remove leftover blank margin or line around the container */
                display: flex !important;
            }
            /* Hide the first child element within the radio group.  Streamlit
               sometimes renders an empty label or wrapper inside the radio
               container which leaves an undesired blank space or small box
               beneath the header.  Hiding this element removes the extra
               whitespace and ensures that only the styled radio buttons are
               visible. */
            div[data-testid="stRadio"] > div:first-child {
                display: none !important;
            }
            /* Also remove any extra padding/margin inside direct child divs of the radio group */
            div[data-testid="stRadio"] > div {
                padding: 0 !important;
                margin: 0 !important;
            }
            /* Adjust header text sizes.  The main header is slightly smaller than before. */
            .main-header {
                font-size: 28px;
                font-weight: 700;
                margin-bottom: 4px;
        ...
        # we keep all plants; otherwise we filter to the chosen plant only.  If the
        # first option is selected but named differently (e.g. "전체"), treat it as
        # selecting all plants.  This makes the behaviour robust to legacy
        # configurations.
        if selected_plant in ("전체", "전체 공장"):
            filtered_df = df.copy()
        else:
        ...
        if selected_week != "전체":
            ...
            try:
                # Parse selected week label like 'W12' to its numeric part
                _week_num = int(str(selected_week).lstrip('W'))
            except Exception:
                _week_num = None
            if _week_num is not None:
                # Extract numeric week values from the data.  Use expand=False
                # to obtain a Series rather than a DataFrame and avoid
                # indexing the first column.  Convert to numeric values
                # (floats) and drop missing entries.  Compare against
                # the selected week number to build a mask.
                _week_col_numeric = pd.to_numeric(
                    filtered_df["주차"].astype(str).str.extract(r"(\d+)", expand=False),
                    errors="coerce"
                )
        ...
        if 'selected_defect_type' in locals() and selected_defect_type != '전체':
            # Filter records by defect type.  Use contains() to match
            # partial strings so that selecting "엣지" also returns records
            # whose classification includes that substring.  If the
            # classification column does not exist, skip filtering.
            if '신규분류요약' in df_for_summaries.columns:
                df_for_summaries = df_for_summaries[
                    df_for_summaries['신규분류요약']
                        .astype(str)
                        .str.contains(str(selected_defect_type), na=False)
                ]
        ...
        #
        # Present the radio button without an outer container so that each option
        # appears as its own bordered button.  Use an empty label to avoid
        # displaying a separate caption; the label text will instead be
        # incorporated into the styled radio options via CSS.
        # Present the dashboard selector without an outer container or blank label.
        # Use label_visibility='collapsed' to hide Streamlit's default label area,
        # otherwise Streamlit reserves space for an empty caption which appears as
        # a small square or line. Each option is styled individually via CSS.
        # Render the dashboard selector without a label.  An empty string for the
        # label combined with the default label visibility prevents Streamlit from
        # reserving space for a caption.  Each option is styled via custom CSS
        # defined above, and appears as its own bordered button.  Without a
        # caption, no extra container or blank line appears beneath the title.
        main_tab = st.radio(
            "",
            ["수율 대시보드", "불량 대시보드"],
            horizontal=True,
        ...
        # Set a fixed y-axis range for all yield charts.  The slider has been removed
        # in favour of a consistent range across all graphs.
        ...
        def summarise_and_percent(
            ...
            # Drop the plant level from the MultiIndex (if present) to simplify the table
            try:
                summary = summary.droplevel("공장")
            except Exception:
                pass
            # If there are duplicate index values (e.g., multiple plants for the same group),
            # average the yields across duplicates so that each time period appears once.
            if summary.index.duplicated().any():
                summary = summary.groupby(summary.index).mean()
            # Convert yields to percentage after averaging
            summary = summary * 100.0
            summary = summary.round(2)
            # Reorder process columns to a consistent order.  Columns not found
            # in the specified order are appended unchanged.  This ensures
            # yield tables and charts display processes in the desired sequence.
            desired_order = [
                '사출조립',
                '분리',
                '하이드레이션/전면검사',
            ...
            existing = [c for c in desired_order if c in summary.columns]
        ...
        # Compute weekly summary by grouping directly on the numeric week.  This
        # avoids constructing a combined year-week key and ensures that newly
        # added weeks (e.g., W12, W13) are automatically included.  Extract the
        # numeric part of the '주차' column, drop rows without a week number,
        # then group by this numeric week.
        df_week = df_for_summaries.copy()
        df_week['주차_num'] = df_week['주차'].astype(str).str.extract(r'(\d+)', expand=False)
        df_week = df_week.dropna(subset=['주차_num'])
        weekly_summary = summarise_and_percent(df_week, group_cols=('주차_num',))
        ...
        # Sort the weekly summary by numeric week and rename index labels to 'W#'.
        if not weekly_summary.empty:
            try:
                week_nums = weekly_summary.index.to_series().astype(str).str.extract(r'(\d+)', expand=False).astype(int)
                sorted_pos = week_nums.sort_values().index
                weekly_summary = weekly_summary.loc[sorted_pos]
                weekly_summary.index = week_nums.loc[sorted_pos].apply(lambda w: f'W{w}')
            except Exception:
                pass
        ...
        def draw_line_chart(summary: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # Round yield values to one decimal place so that both the
                # markers (text labels) and hover values display one decimal.
                df_long["수율"] = df_long["수율"].round(1)
                ...
                # Fix the y-axis range to a consistent scale
                fig.update_yaxes(range=[y_min, y_max])
                # Set x-axis category order explicitly to ensure chronological order.
                # Without this, Plotly may reorder categories alphabetically (e.g., W12
                # before W2).  We use the DataFrame index order for the category array.
                fig.update_xaxes(
                    type="category",
                    categoryorder="array",
                    categoryarray=list(summary.index)
                )
                ...
                # Basic layout: show legend title and axis labels
                fig.update_layout(
        ...
        def draw_product_chart(pivot_df: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # Set y-axis range to 50–100 to match other yield charts
                fig.update_yaxes(range=[50, 100])
                fig.update_layout(legend_title_text="제품군")
                # Show data labels above points
                fig.update_traces(text=df_long["수율"], textposition="top center")
                st.plotly_chart(fig, use_container_width=True)
            else:
        ...
        def compute_combined_summaries(full_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
            ...
            def _compute_yields(group: pd.DataFrame) -> float:
                # Aggregate good and production quantities per process
                proc_agg = group.groupby('Process').agg(
                    good=('양품수량', 'sum'),
                    prod=('생산수량', 'sum'),
                )
                # Compute per-process yields (handle zero production gracefully)
                yields = []
                for proc in PROCESS_ORDER:
                    if proc in proc_agg.index and proc_agg.loc[proc, 'prod'] != 0:
                        yields.append(proc_agg.loc[proc, 'good'] / proc_agg.loc[proc, 'prod'])
                    else:
                        yields.append(1.0)
                # Overall yield is the product of per-process yields
                result = float(np.prod(yields))
                return result
        ...
        def draw_combined_chart(combined_df: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # Define distinct colors for the yield lines for better contrast
                line_colors = ['#e6194b', '#3cb44b', '#4363d8', '#f58231']
                ...
                for col, color in zip(['A관', 'C관', 'S관', '종합수율'], line_colors):
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            text=[f"{val:.1f}" for val in line_values],
                ...
                # Left y-axis for yield (%) with fixed range
                fig.update_yaxes(title_text='수율 (%)', secondary_y=False, range=[50, 100])
                # Right y-axis for quantity (in millions)
                fig.update_yaxes(title_text='수량 (M)', secondary_y=True)
                st.plotly_chart(fig, use_container_width=True)
            else:
                ...
                for col, color in zip(['A관', 'C관', 'S관', '종합수율'], line_colors):
                    ...
                    ax_yield.plot(x, y_vals, marker='o', label=col, color=color, zorder=3)
                    # Annotate each point with its value (one decimal, no %)
                ...
                ax_yield.set_ylabel('수율 (%)')
                ax_yield.set_ylim(50, 100)
                # Set x-axis labels
                ax_yield.set_xticks(x)
                ax_yield.set_xticklabels(combined_df.index, rotation=45)
        ...
        with tab_weekly:
            ...
            # Draw the weekly summary chart.  Do not truncate the data so that
            # all available weeks (including newly added weeks such as W12, W13, etc.)
            # are displayed.  Previously the chart showed only the most recent
            # 10 weeks, which caused newer weeks to be hidden once the dataset
            # grew beyond 10 weeks.
        ...
        with tab_prod:
            ...
            if USE_PLOTLY:
                ...
                # Set the yield axis to range between 50 and 100 to match other yield charts
                fig.update_yaxes(title_text='수율 (%)', secondary_y=True, range=[50, 100])
                st.plotly_chart(fig, use_container_width=True)
            else:
        ...
        if not category_columns:
            ...
            # Display the filtered product yield table first so that users see the raw numbers
            ...
            # Then draw the line chart for the selected category and period
            draw_product_chart(
                pivot_filtered,
