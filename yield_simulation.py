...
def main() -> None:
    ...
    # First look for known filenames in the directory of this script.
    for name in candidates:
        candidate_path = os.path.join(os.path.dirname(__file__), name)
        if os.path.exists(candidate_path):
            file_path = candidate_path
            break
    # If no known file is found, search for any matching workbook in the
    # current directory.  Prefer files containing '수율' in the name
    # (indicating yield data) over other matches.  If multiple matches
    # exist, choose the one with the most recent modification time.
    if file_path is None:
        dirname = os.path.dirname(__file__)
        candidates_all = []
        for fname in os.listdir(dirname):
            if fname.startswith("공정기술팀 대시보드") and fname.endswith(".xlsx"):
                candidates_all.append(fname)
        if candidates_all:
            # Separate yield-specific files from general files
            yield_files = [f for f in candidates_all if "수율" in f]
            search_list = yield_files or candidates_all
    ...
    if file_path is None:
        print(
            ...
            "'공정기술팀 대시보드' and ending with '.xlsx' in the same directory as this script."
        )
        return
    ...
    if st:
        ...
        if "주차" in df.columns:
            try:
                week_num_series = (
                    df["주차"]
                    .astype(str)
                    .str.extract(r"(\d+)", expand=False)[0]
                    .dropna()
                    .astype(int)
                )
                unique_weeks = week_num_series.unique().tolist()
                unique_weeks.sort()
                weeks = [f"W{w}" for w in unique_weeks]
            except Exception:
                weeks = []
        ...
        defect_plants = ["전체 공장"] + sorted(df["공장"].unique().tolist())
        ...
        # Reorder weekly summary: sort by numeric week number and rename the
        # index labels to 'W#'.  Do not drop any week numbers so that all
        # available weeks (including W1, W12, W13, etc.) appear.  Weeks are
        # extracted from the 'year_week' index which has the format
        # 'YYYY-<week>'.  Sorting by the numeric component ensures W10/W11
        # appear after W9 and that newly added weeks automatically append.
        if not weekly_summary.empty:
            try:
                # Extract year and week components from index entries of the form
                # "YYYY-W#" (e.g., "2026-12").  Convert both to integers for
                # sorting.  When multiple years are present, sorting by
                # (year, week) ensures that earlier years appear to the left and
                # later years to the right.  Stable sorting preserves the
                # relative order of duplicate week numbers within the same year.
                idx_series = weekly_summary.index.to_series()
                year_part = idx_series.str.extract(r"^(\d+)", expand=False).astype(int)
                week_part = idx_series.str.extract(r"-(\d+)", expand=False).astype(int)
                sort_keys = list(zip(year_part, week_part))
                # Obtain the sorted positions by enumeration and sorting by keys
                sorted_positions = sorted(range(len(sort_keys)), key=lambda i: sort_keys[i])
                weekly_summary = weekly_summary.iloc[sorted_positions]
                # Construct new labels in W# format based on the week part
                new_labels = [f"W{week_part.iloc[i]}" for i in sorted_positions]
                weekly_summary.index = new_labels
            except Exception:
                # In case of any parsing error, leave the weekly summary unchanged
                pass
