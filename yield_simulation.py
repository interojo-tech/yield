...
def main() -> None:
    ...
    def _load_and_prepare(path: str) -> pd.DataFrame:
        ...
        # Sort by production date so that new data appears in chronological order.
        # Reset the index to keep a clean integer index after sorting.
    ...
    if st:
        ...
        # Streamlit caching for data loading.  Include the file's modification
        # time in the cache key so that updates to the Excel file invalidate
        # the cached data.  The mtime argument is not used inside the
        # function body but is part of the cache signature.
