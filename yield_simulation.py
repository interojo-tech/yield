...
def main() -> None:
    ...
    if st:
        ...
        # Fix the yield y-axis range.  When viewing the entire plant ('전체 공장'),
        # constrain the range to 80–100%.  Otherwise use the default 50–100% range.
        # Removing the slider prevents users from adjusting this range.  This
        # conditional assignment ensures that the overall plant charts focus on
        # higher yield values while individual plant views retain the wider range.
        if selected_plant == "전체 공장":
        ...
        # Do not drop any weeks; simply map the existing index to 'W#'.  This
        # prevents missing week entries that may occur if week 1 or other
        # numbers are filtered out.  Sorting ensures that W2, W3, ..., W12
        # appear in natural order.  If an error occurs (e.g., unexpected index
        # format), fall back to leaving the index unchanged.
        if not weekly_summary.empty:
            try:
                # weekly_summary.index format is 'YYYY-<week>' (e.g., '2026-2').
        ...
        def draw_line_chart(summary: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # Set axis colours to black (ticks and axis lines)
