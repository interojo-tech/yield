...
def main() -> None:
    ...
    if st:
        ...
        if last_date_label:
            # Use HTML to adjust font size of the date portion.  The main header
            # matches the size of a Streamlit subheader, and the date text
            # appears slightly smaller after a hyphen.
            st.markdown(
                f"<h3 style='margin-bottom:0;'>종합 수율 및 양품량 <span style='font-size:0.8em; font-weight:500;'>- Data 기준일: {last_date_label}</span></h3>",
                unsafe_allow_html=True,
            )
        else:
