...
def main() -> None:
    ...
    if st:
        ...
        def reorder_summary_table(df: pd.DataFrame, first_col: str) -> pd.DataFrame:
            """Reorder summary table columns by the requested process order."""
            ordered = [first_col, "사출조립", "분리", "하이드레이션/전면검사", "접착/멸균", "누수/규격검사", "overall_yield"]
        ...
        def draw_line_chart(summary: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                process_order_local = ["사출조립", "분리", "하이드레이션/전면검사", "접착/멸균", "누수/규격검사", "종합수율"]
                ...
                fig = px.line(
                    ...
                    y="수율",
                    color="공정",
                    category_orders={"공정": ["사출조립", "분리", "하이드레이션/전면검사", "접착/멸균", "누수/규격검사", "종합수율"]},
                    markers=True,
                    title=title,
        ...
        with tab_daily:
            ...
            daily_df = summary_daily.reset_index().rename(columns={"index": "날짜"})
            daily_df = reorder_summary_table(daily_df, "날짜")
            ...
            if not summary_daily.empty:
                if selected_plant == "전체":
                    for plant_name in sorted(filtered_df["공장"].unique().tolist()):
                        plant_df = filtered_df[filtered_df["공장"] == plant_name].copy()
                        plant_daily = summarise_and_percent(plant_df, group_cols=("생산일자",))
                        plant_daily.index = plant_daily.index.strftime("%Y-%m-%d")
                        if selected_date_daily != '전체':
                            if selected_date_daily in plant_daily.index:
                                plant_daily = plant_daily.loc[[selected_date_daily]]
                            else:
                                plant_daily = plant_daily.iloc[0:0]
                        if not plant_daily.empty:
                            st.markdown(f"**{plant_name}**")
                            draw_line_chart(plant_daily, f"{plant_name} 일별 수율 (공정별)", "날짜")
                else:
            ...
            if not summary_weekly.empty:
                if selected_plant == "전체":
                    for plant_name in sorted(filtered_df["공장"].unique().tolist()):
                        ...
                        plant_week["year_week"] = plant_week["년도"].astype(int).astype(str) + "-" + plant_week["주차"].astype(str)
                        plant_weekly = summarise_and_percent(plant_week, group_cols=("year_week",))
                        if selected_date_weekly != '전체':
                            dt_sel = pd.to_datetime(selected_date_weekly)
                            week_no = int(dt_sel.isocalendar().week)
                            week_key = f"{dt_sel.year}-{week_no}"
                            if week_key in plant_weekly.index:
                                plant_weekly = plant_weekly.loc[[week_key]]
                            else:
                                plant_weekly = plant_weekly.iloc[0:0]
                        if not plant_weekly.empty:
                            st.markdown(f"**{plant_name}**")
                            draw_line_chart(plant_weekly, f"{plant_name} 주간 수율 (공정별)", "주차")
                else:
        ...
        with tab_monthly:
            ...
            if not summary_monthly.empty:
                if selected_plant == "전체":
                    for plant_name in sorted(filtered_df["공장"].unique().tolist()):
                        plant_df = filtered_df[filtered_df["공장"] == plant_name].copy()
                        plant_monthly = summarise_and_percent(plant_df, group_cols=("년도", "월"))
                        plant_monthly.index = (
                            plant_monthly.index.get_level_values("년도").astype(int).astype(str)
                            + "-"
                            + plant_monthly.index.get_level_values("월").astype(str)
                        )
                        if selected_date_monthly != '전체':
                            dt_sel_m = pd.to_datetime(selected_date_monthly)
                            month_key = f"{dt_sel_m.year}-{dt_sel_m.month}"
                            if month_key in plant_monthly.index:
                                plant_monthly = plant_monthly.loc[[month_key]]
                            else:
                                plant_monthly = plant_monthly.iloc[0:0]
                        if not plant_monthly.empty:
                            st.markdown(f"**{plant_name}**")
                            draw_line_chart(plant_monthly, f"{plant_name} 월별 수율 (공정별)", "연-월")
                else:
