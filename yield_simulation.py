...
def main() -> None:
    ...
    if st:
        ...
        with tab_prod:
            # Production quantity check: chart first then table with yield axis on left and quantity axis on right
            ...
            # Show chart before table
            if USE_PLOTLY:
                from plotly.subplots import make_subplots  # type: ignore
                import plotly.graph_objects as go  # type: ignore
                fig = make_subplots(specs=[[{'secondary_y': True}]])
                # Bars for quantities on secondary (right) y-axis with labels
                fig.add_trace(
                    go.Bar(
                        x=prod_df['Process'],
                        y=prod_df['생산량'],
                ...
                # Line for yield on primary (left) y-axis with markers and labels
                fig.add_trace(
                    go.Scatter(
                        x=prod_df['Process'],
                        y=prod_df['수율(%)'],
                ...
                # Matplotlib fallback: line on primary axis (yield), bars on secondary axis (quantity)
                fig, ax1 = plt.subplots(figsize=(8, 4))
                processes = prod_df['Process']
                x = np.arange(len(processes))
                width = 0.35
                # Plot yield line on primary axis with labels
                ax1.plot(x, prod_df['수율(%)'], color='#ee6666', marker='o', label='수율(%)')
                ax1.set_xlabel('공정')
                ax1.set_ylabel('수율 (%)')
                ax1.set_ylim(50, 100)
                for i, v in enumerate(prod_df['수율(%)']):
                    ax1.annotate(f"{v}%", (x[i], v), textcoords='offset points', xytext=(0, 5), ha='center', color='#ee6666')
                # Secondary axis for quantities
                ax2 = ax1.twinx()
                ax2.bar(x - width/2, prod_df['생산량'], width=width, label='생산량', color='#5470c6')
                ax2.bar(x + width/2, prod_df['양품량'], width=width, label='양품량', color='#91cc75')
                ax2.set_ylabel('수량')
                for i, v in enumerate(prod_df['생산량']):
                    ax2.annotate(f"{v:,}", (x[i] - width/2, v), textcoords='offset points', xytext=(0, 5), ha='center', color='#5470c6')
                for i, v in enumerate(prod_df['양품량']):
                    ax2.annotate(f"{v:,}", (x[i] + width/2, v), textcoords='offset points', xytext=(0, 5), ha='center', color='#91cc75')
                ...
                fig.tight_layout()
                st.pyplot(fig)
            # Display the table after the chart
