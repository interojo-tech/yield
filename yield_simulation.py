    ...
    if st:
        ...
        def compute_combined_summaries(full_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
            ...
            if not weekly_combined.empty:
                # Sort by year and week number, then create labels like '2026-W05'.
                weekly_combined = weekly_combined.sort_values(['년도', '주차번호'])
                weekly_combined['주차'] = weekly_combined.apply(
                    lambda row: f"{int(row['년도'])}-W{int(row['주차번호']):02d}", axis=1
                )
        ...
        # Only for the combined weekly chart, show the latest 10 weeks from the selected year.
        weekly_combined_plot = weekly_combined.copy()
        if not weekly_combined_plot.empty:
            try:
                _wc = weekly_combined_plot.reset_index().rename(columns={weekly_combined_plot.index.name or 'index': '주차'})
                _wc['년도_str'] = _wc['주차'].astype(str).str.extract(r'^(\d{4})-W\d+$')[0]
                _wc['주차번호'] = pd.to_numeric(
                    _wc['주차'].astype(str).str.extract(r'W(\d+)$')[0], errors='coerce'
                )
                _target_year = str(selected_year) if selected_year != '전체' else '2026'
                _wc = _wc[_wc['년도_str'] == _target_year]
                _wc = _wc.sort_values(['년도_str', '주차번호'])
                _wc = _wc.tail(min(10, len(_wc)))
                weekly_combined_plot = _wc.set_index('주차')[['양품량', 'A관', 'C관', 'S관', '종합수율']]
            except Exception:
                weekly_combined_plot = weekly_combined.tail(min(10, len(weekly_combined)))
        ...
        with comb_cols[1]:
            ...
            draw_combined_chart(weekly_combined_plot, "주간 양품량 및 수율", "주차", (65.0, 85.0))
