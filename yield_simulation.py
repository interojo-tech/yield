...
def main() -> None:
    ...
    if st:
        ...
        def reorder_summary_table(df: pd.DataFrame, first_col: str) -> pd.DataFrame:
            """Reorder yield summary columns and rename overall_yield to 수율."""
            ...
            if "overall_yield" in renamed.columns:
                ...
                ordered = [first_col, "사출조립", "분리", "하이드레이션/전면검사", "접착/멸균", "누수/규격검사", "수율"]
            existing = [c for c in ordered if c in renamed.columns]
            others = [c for c in renamed.columns if c not in existing]
            return renamed[existing + others]
        ...
        def draw_line_chart(summary: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                fig = px.line(
                    ...
                    y="수율",
                    color="공정",
                    category_orders={"공정": process_order_local},
                    markers=True,
                    title=title,
                ...
                fig.update_layout(
                    legend_title_text="공정", xaxis_title=xlabel, yaxis_title="수율 (%)", dragmode="zoom"
                )
                st.plotly_chart(fig, use_container_width=True, config={"scrollZoom": True, "displaylogo": False})
            else:
                process_cols = [c for c in ["사출조립", "분리", "하이드레이션/전면검사", "접착/멸균", "누수/규격검사"] if c in summary.columns]
