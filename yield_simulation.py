...
def main() -> None:
    ...
    if st:
        ...
        # For weeks, convert to integers, sort ascending and remove week numbers < 2.
        week_numbers = sorted(df["주차"].dropna().astype(int).unique().tolist())
        week_numbers = [w for w in week_numbers if w >= 2]
        # Convert back to strings for display
        weeks = [str(w) for w in week_numbers]
        ...
        # Reorder weekly summary index and drop weeks before W2 (week number < 2)
        if not weekly_summary.empty:
            def _week_num(idx_str: str) -> int:
                try:
                    # index may look like 'YYYY-W#' or 'YYYY-W##'
                    return int(str(idx_str).split("-")[-1])
                except Exception:
                    return 0
            sorted_week_idx = sorted(weekly_summary.index.tolist(), key=_week_num)
            sorted_week_idx = [i for i in sorted_week_idx if _week_num(i) >= 2]
            weekly_summary = weekly_summary.loc[sorted_week_idx]
        ...
        if not product_weekly_pivot.empty:
            ...
            # Filter out any week labels whose numeric part is less than 2
            sorted_week_index = [idx for idx in sorted_week_index if _extract_week(idx) >= 2]
        ...
        # ------------------------------------------------------------------
        # Use session state to remember the toggle state across reruns.  When the
        # button is clicked, flip the state immediately and update the button label
        if 'show_data' not in st.session_state:
            st.session_state['show_data'] = False
        # Determine label based on current state
        data_button_label = '전체 데이터 숨기기' if st.session_state['show_data'] else '전체 데이터 보기'
        # Render button and update state when clicked
        if st.button(data_button_label):
            st.session_state['show_data'] = not st.session_state['show_data']
        # Show the filtered dataset if the state is True
