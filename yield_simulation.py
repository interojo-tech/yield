...
def main() -> None:
    ...
    if st:
        ...
        # Toggle to show or hide the full filtered dataset.  Use session state
        # to remember the toggle state across reruns.  The button label
        # switches depending on whether the dataset is currently visible.
        if 'show_data' not in st.session_state:
            st.session_state['show_data'] = False
        data_toggle_label = '전체 데이터 숨기기' if st.session_state['show_data'] else '전체 데이터 보기'
        # When the button is clicked, immediately flip the state
        if st.button(data_toggle_label):
        ...
        # ------------------------------------------------------------------
        # Product-level yield views (daily, weekly, monthly)
        # ------------------------------------------------------------------
        st.subheader("제품군별 수율 현황")
        # Provide buttons (radio) for selecting the time granularity
        product_period = st.radio(
            "기간 선택",
            ["일별", "주간", "월별"],
            horizontal=True,
            index=0,
        )
        if product_period == "일별":
            # Daily product yield table and chart
            idx_name = product_daily_pivot.index.name or "_day_key"
            df_view = product_daily_pivot.reset_index().rename(columns={idx_name: "날짜"})
            st.dataframe(df_view, use_container_width=True)
            draw_product_chart(product_daily_pivot, f"{selected_plant} - 제품군별 일별 수율", "날짜")
        elif product_period == "주간":
            # Weekly product yield table and chart
            idx_name = product_weekly_pivot.index.name or "_week_key"
            df_view = product_weekly_pivot.reset_index().rename(columns={idx_name: "연-주"})
            st.dataframe(df_view, use_container_width=True)
            draw_product_chart(product_weekly_pivot, f"{selected_plant} - 제품군별 주간 수율", "연-주")
        else:
            # Monthly product yield table and chart
            idx_name = product_monthly_pivot.index.name or "_month_key"
            df_view = product_monthly_pivot.reset_index().rename(columns={idx_name: "연-월"})
            st.dataframe(df_view, use_container_width=True)
            draw_product_chart(product_monthly_pivot, f"{selected_plant} - 제품군별 월별 수율", "연-월")
    else:
