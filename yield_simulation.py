...
def main() -> None:
    ...
    if st:
        ...
        # ------------------------------------------------------------------
        # Maintain a single toggle state in session_state.  Clicking the button flips
        # the state and immediately updates the button label.  Use a fixed key on
        # the button to ensure consistent behaviour across reruns.
        if 'show_data' not in st.session_state:
            st.session_state.show_data = False
        # Determine the button label based on current visibility
        show_label = '전체 데이터 숨기기' if st.session_state.show_data else '전체 데이터 보기'
        # Place the button; when clicked, flip the state
        if st.button(show_label, key='toggle_data_button'):
            st.session_state.show_data = not st.session_state.show_data
        # If visible, display the full dataset along with a header
