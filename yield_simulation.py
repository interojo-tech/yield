...
def _safe_year_str(values) -> pd.Series:
    """Safely convert year-like values to strings, preserving missing values."""
    return (
...
def main() -> None:
    ...
    if st:
        ...
        try:
            ...
            if selected_year != "전체":
                tmp_df = tmp_df[_safe_year_str(tmp_df["년도"]) == selected_year]
        ...
        # Daily date range picker.  Use None as the initial value so that
        # selecting '선택없음' (no selection) is supported.  We also bound the
        # selectable dates using the minimum and maximum dates from tmp_df.  If
        # these are unavailable, no bounds are provided.  The date picker is
        # positioned below the weekly selectors per the requested layout.
