...
def discover_yield_files(base_dir: str) -> List[str]:
    """Discover yield Excel files in the app directory.

    Priority:
    1. Quarterly files like 공정기술팀 대시보드(수율)_25.1q.xlsx
    2. Legacy single-workbook filenames
    """
    quarterly_patterns = [
        os.path.join(base_dir, '공정기술팀 대시보드(수율)_*.xlsx'),
        os.path.join(base_dir, '공정기술팀 대시보드(수율)_*q.xlsx'),
    ]
    quarter_re = re.compile(r'공정기술팀 대시보드\(수율\)_(\d{2})\.(\d)q\.xlsx$', re.IGNORECASE)
    quarter_files: List[str] = []
    for pattern in quarterly_patterns:
        quarter_files.extend(glob.glob(pattern))
    quarter_files = sorted(set(quarter_files), key=lambda p: (
        int(quarter_re.search(os.path.basename(p)).group(1)) if quarter_re.search(os.path.basename(p)) else -1,
        int(quarter_re.search(os.path.basename(p)).group(2)) if quarter_re.search(os.path.basename(p)) else -1,
        os.path.basename(p),
    ))
    if quarter_files:
    ...
    for name in legacy_candidates:
        ...
        if os.path.exists(candidate_path):
            legacy_files.append(candidate_path)
            break
    return legacy_files


def load_and_merge_data(file_paths: List[str]) -> pd.DataFrame:
    """Load one or more yield workbooks and merge them into one DataFrame."""
    frames: List[pd.DataFrame] = []
    for file_path in file_paths:
        df_part = load_data(file_path)
...
def main() -> None:
    ...
    """
    # Determine the workbook paths. Prefer quarterly files such as
    # '공정기술팀 대시보드(수율)_25.1q.xlsx'. If no quarterly files are present,
    # fall back to the legacy single-workbook filenames.
    base_dir = os.path.dirname(__file__)
    file_paths = discover_yield_files(base_dir)
    if not file_paths:
        message = (
            "수율 데이터 파일을 찾을 수 없습니다. "
            "예: '공정기술팀 대시보드(수율)_25.1q.xlsx' 또는 기존 파일명을 "
            "yield_simulation.py와 같은 폴더에 업로드해 주세요."
        )
        if st:
            st.error(message)
        else:
    ...
    # interaction.
    def _load_and_prepare_from_df(df_local: pd.DataFrame) -> pd.DataFrame:
        if df_local.empty:
            return df_local
    ...
    def _load_and_prepare(path: str) -> pd.DataFrame:
        return _load_and_prepare_from_df(load_data(path))
    ...
    if st:
        ...
        def load_cached(paths: Tuple[str, ...]) -> pd.DataFrame:  # pragma: no cover
            merged = load_and_merge_data(list(paths))
            return _load_and_prepare_from_df(merged)
        ...
        default_year_index = year_options.index("2026") if "2026" in year_options else 0
        selected_year = st.sidebar.selectbox("년도 선택", options=year_options, index=default_year_index)
        ...
        if selected_year != "전체":
            filtered_df = filtered_df[pd.to_numeric(filtered_df["년도"], errors="coerce").astype("Int64").astype(str) == selected_year]
        ...
        #
        # Compute the earliest and latest production dates in the filtered dataset.  We
        # use these to set the default values for the date pickers.  Convert
        # to Python date objects for compatibility with st.date_input.
        try:
            ...
            _min_dt = None
            _max_dt = None
        # Daily date range picker: allow the user to choose a start and end date.
        # When the user selects two dates, Streamlit returns a tuple of two
        # datetime.date objects.  If only a single date is selected, Streamlit
        # returns a single date; we normalise to a tuple below.
        date_range_daily = st.sidebar.date_input(
            "일별 기간 선택",
            value=(_min_dt, _max_dt) if _min_dt and _max_dt else None,
            key='date_range_daily',
        )
        # Weekly date range picker: similar to daily.  This allows the user to
        # filter the weekly summary to a specific date span.  If no valid
        # default range exists, use None so that the widget displays a
        # placeholder and the subsequent logic treats it as unbounded.
        ...
        def _sort_limit_monthly_df(summary: pd.DataFrame, limit: int | None = None) -> pd.DataFrame:
            if summary is None or summary.empty:
                return summary
            tmp = summary.copy()
            meta = tmp.index.to_series().astype(str).str.extract(r"(?P<year>\d{4})[^\d]?(?P<month>\d{1,2})")
            meta['year'] = pd.to_numeric(meta['year'], errors='coerce')
            meta['month'] = pd.to_numeric(meta['month'], errors='coerce')
            tmp['_sort_key'] = pd.to_datetime(
                {
                    'year': meta['year'].fillna(1900).astype(int),
                    'month': meta['month'].fillna(1).astype(int),
                    'day': 1,
                },
                errors='coerce'
            )
            tmp = tmp.sort_values('_sort_key')
            tmp.index = tmp['_sort_key'].dt.strftime('%Y-%m')
            tmp = tmp.drop(columns=['_sort_key'])
            if limit is not None and len(tmp) > limit:
                tmp = tmp.tail(limit)
            return tmp

        def _sort_limit_weekly_df(summary: pd.DataFrame, limit: int | None = None) -> pd.DataFrame:
            if summary is None or summary.empty:
                return summary
            tmp = summary.copy()
            meta = tmp.index.to_series().astype(str).str.extract(r"(?P<year>\d{4})-W(?P<week>\d{1,2})")
            meta['year'] = pd.to_numeric(meta['year'], errors='coerce')
            meta['week'] = pd.to_numeric(meta['week'], errors='coerce')
            # ISO week sorting anchor: Jan 4 is always in ISO week 1.
            sort_base = pd.to_datetime(meta['year'].fillna(1900).astype(int).astype(str) + '-01-04', errors='coerce')
            tmp['_sort_key'] = sort_base + pd.to_timedelta((meta['week'].fillna(1).astype(int) - 1) * 7, unit='D')
            tmp = tmp.sort_values('_sort_key')
            iso_year = tmp['_sort_key'].dt.isocalendar().year.astype(int)
            iso_week = tmp['_sort_key'].dt.isocalendar().week.astype(int)
            tmp.index = [f"{y}-W{w:02d}" for y, w in zip(iso_year, iso_week)]
            tmp = tmp.drop(columns=['_sort_key'])
            if limit is not None and len(tmp) > limit:
                tmp = tmp.tail(limit)
            return tmp
        ...
        # Use the defect‑filtered dataset for weekly summaries.  Keep year and week together
        # so that recent-week slicing does not accidentally mix 2025 and 2026 data.
        df_week = df_for_summaries[
            (df_for_summaries["생산일자"].dt.date >= start_w)
            & (df_for_summaries["생산일자"].dt.date <= end_w)
        ].copy()
        df_week["year_num"] = pd.to_numeric(df_week["년도"], errors="coerce")
        df_week["week_num"] = pd.to_numeric(
            df_week["주차"].astype(str).str.extract(r"(\d+)")[0], errors="coerce"
        )
        df_week = df_week[df_week["year_num"].notna() & df_week["week_num"].notna()].copy()
        df_week["year_num"] = df_week["year_num"].astype(int)
        df_week["week_num"] = df_week["week_num"].astype(int)
        weekly_summary = summarise_and_percent(df_week, group_cols=("year_num", "week_num"))
        if 'selected_defect_process' in locals() and selected_defect_process != '전체':
        ...
        if not weekly_summary.empty:
            try:
                idx_frame = weekly_summary.index.to_frame(index=False)
                idx_frame.columns = ['year_num', 'week_num']
                idx_frame['year_num'] = pd.to_numeric(idx_frame['year_num'], errors='coerce').fillna(1900).astype(int)
                idx_frame['week_num'] = pd.to_numeric(idx_frame['week_num'], errors='coerce').fillna(1).astype(int)
                sort_key = pd.to_datetime(idx_frame['year_num'].astype(str) + '-01-04', errors='coerce') + pd.to_timedelta((idx_frame['week_num'] - 1) * 7, unit='D')
                weekly_summary = weekly_summary.copy()
                weekly_summary['_sort_key'] = sort_key.values
                weekly_summary = weekly_summary.sort_values('_sort_key')
                iso_year = weekly_summary['_sort_key'].dt.isocalendar().year.astype(int)
                iso_week = weekly_summary['_sort_key'].dt.isocalendar().week.astype(int)
                weekly_summary.index = [f"{y}-W{w:02d}" for y, w in zip(iso_year, iso_week)]
                weekly_summary.index.name = '주차'
                weekly_summary = weekly_summary.drop(columns=['_sort_key'])
            except Exception:
                weekly_summary.index = weekly_summary.index.map(str)

        monthly_summary = summarise_and_percent(df_for_summaries, group_cols=("년도", "월"))
        if 'selected_defect_process' in locals() and selected_defect_process != '전체':
        ...
        if not monthly_summary.empty:
            try:
                ...
                year_vals = pd.to_numeric(idx_frame.iloc[:, 0], errors='coerce').fillna(1900).astype(int)
                month_vals = idx_frame.iloc[:, 1].apply(_extract_month_num).fillna(1).astype(int)
                sort_key = pd.to_datetime(
                    {'year': year_vals, 'month': month_vals, 'day': 1},
                    errors='coerce'
                )
                monthly_summary = monthly_summary.copy()
                monthly_summary['_sort_key'] = sort_key.values
                monthly_summary = monthly_summary.sort_values('_sort_key')
                monthly_summary.index = monthly_summary['_sort_key'].dt.strftime('%Y-%m')
                monthly_summary = monthly_summary.drop(columns=['_sort_key'])
            except Exception:
        ...
        _prod_df["_week_key"] = (
            pd.to_numeric(_prod_df["년도"], errors="coerce").fillna(1900).astype(int).astype(str)
            + "-W"
            + pd.to_numeric(_prod_df["주차"].astype(str).str.extract(r"(\d+)")[0], errors="coerce").fillna(0).astype(int).astype(str).str.zfill(2)
        )
        # Monthly key: year-month format
        _prod_df["_month_key"] = (
            pd.to_numeric(_prod_df["년도"], errors="coerce").fillna(1900).astype(int).astype(str)
            + "-"
            + _prod_df["월"].apply(_extract_month_num).fillna(1).astype(int).astype(str).str.zfill(2)
        )
        ...
        # Reorder weekly pivot by year-week and keep only the recent 10 weeks.
        if not product_weekly_pivot.empty:
            product_weekly_pivot = _sort_product_weekly_pivot(product_weekly_pivot)
            product_weekly_pivot = product_weekly_pivot.tail(min(10, len(product_weekly_pivot)))
        ...
        def compute_combined_summaries(full_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
            ...
            if not monthly_combined.empty:
                monthly_combined['_month_num'] = monthly_combined['월'].apply(_extract_month_num)
                monthly_combined['_sort_key'] = pd.to_datetime(
                    {
                        'year': pd.to_numeric(monthly_combined['년도'], errors='coerce').fillna(1900).astype(int),
                        'month': monthly_combined['_month_num'].fillna(1).astype(int),
                        'day': 1,
                    },
                    errors='coerce'
                )
                monthly_combined = monthly_combined.sort_values('_sort_key')
                monthly_combined['연-월'] = monthly_combined['_sort_key'].dt.strftime('%Y-%m')
                monthly_combined = monthly_combined.set_index('연-월')[['양품량', 'A관', 'C관', 'S관', '종합수율']]
            else:
                monthly_combined = pd.DataFrame(columns=['양품량', 'A관', 'C관', 'S관', '종합수율'])
            ...
            if not weekly_combined.empty:
                weekly_combined['_sort_key'] = pd.to_datetime(
                    pd.to_numeric(weekly_combined['년도'], errors='coerce').fillna(1900).astype(int).astype(str) + '-01-04',
                    errors='coerce'
                ) + pd.to_timedelta((pd.to_numeric(weekly_combined['주차번호'], errors='coerce').fillna(1).astype(int) - 1) * 7, unit='D')
                weekly_combined = weekly_combined.sort_values('_sort_key')
                iso_year = weekly_combined['_sort_key'].dt.isocalendar().year.astype(int)
                iso_week = weekly_combined['_sort_key'].dt.isocalendar().week.astype(int)
                weekly_combined['주차'] = [f"{y}-W{w:02d}" for y, w in zip(iso_year, iso_week)]
                weekly_combined = weekly_combined.set_index('주차')[['양품량', 'A관', 'C관', 'S관', '종합수율']]
            else:
        ...
        def compute_process_period_summary(full_df: pd.DataFrame, period: str) -> Dict[str, pd.DataFrame]:
            """Compute per-process summaries with chronological labels."""
            result: Dict[str, pd.DataFrame] = {}
            for proc in PROCESS_ORDER:
                proc_df = full_df[full_df["Process"] == proc].copy()
                if proc_df.empty:
                    result[proc] = pd.DataFrame(columns=[proc])
                    continue
                if period == "monthly":
                    proc_df['_year_num'] = pd.to_numeric(proc_df['년도'], errors='coerce')
                    proc_df['_month_num'] = proc_df['월'].apply(_extract_month_num)
                    proc_df = proc_df.dropna(subset=['_year_num', '_month_num']).copy()
                    grouped = (
                        proc_df.groupby(['_year_num', '_month_num'])
                        .agg(양품수량=('양품수량', 'sum'), 생산수량=('생산수량', 'sum'))
                        .reset_index()
                    )
                    grouped['_sort_key'] = pd.to_datetime(
                        {
                            'year': grouped['_year_num'].astype(int),
                            'month': grouped['_month_num'].astype(int),
                            'day': 1,
                        },
                        errors='coerce'
                    )
                    grouped = grouped.sort_values('_sort_key')
                    grouped['label'] = grouped['_sort_key'].dt.strftime('%Y-%m')
                else:
                    proc_df['_year_num'] = pd.to_numeric(proc_df['년도'], errors='coerce')
                    proc_df['_week_num'] = pd.to_numeric(proc_df['주차'].astype(str).str.extract(r"(\d+)")[0], errors='coerce')
                    proc_df = proc_df.dropna(subset=['_year_num', '_week_num']).copy()
                    grouped = (
                        proc_df.groupby(['_year_num', '_week_num'])
                        .agg(양품수량=('양품수량', 'sum'), 생산수량=('생산수량', 'sum'))
                        .reset_index()
                    )
                    grouped['_sort_key'] = pd.to_datetime(
                        grouped['_year_num'].astype(int).astype(str) + '-01-04',
                        errors='coerce'
                    ) + pd.to_timedelta((grouped['_week_num'].astype(int) - 1) * 7, unit='D')
                    grouped = grouped.sort_values('_sort_key')
                    iso_year = grouped['_sort_key'].dt.isocalendar().year.astype(int)
                    iso_week = grouped['_sort_key'].dt.isocalendar().week.astype(int)
                    grouped['label'] = [f"{y}-W{w:02d}" for y, w in zip(iso_year, iso_week)]
                grouped[proc] = np.where(grouped['생산수량'] > 0, grouped['양품수량'] / grouped['생산수량'] * 100.0, 100.0)
                result[proc] = grouped.set_index('label')[[proc]].round(2)
            return result
        ...
        with comb_cols[0]:
            ...
            draw_combined_chart(_sort_limit_monthly_df(monthly_combined, 12), "월간 양품량 및 수율", "연-월", (65.0, 80.0))
        with comb_cols[1]:
            st.markdown("**주간**")
            # For weekly combined chart, use a yield range of 65–85 to capture more variation
            draw_combined_chart(_sort_limit_weekly_df(weekly_combined, 12), "주간 양품량 및 수율", "주차", (65.0, 85.0))
        ...
        with tab_weekly:
            ...
            if not weekly_summary.empty:
                weekly_summary_plot = _sort_limit_weekly_df(weekly_summary, 10)
            else:
            ...
            if selected_date_weekly == '전체':
                ...
                week_key = f"{dt_sel.year}-W{week_no:02d}"
        ...
        with tab_monthly:
            ...
            draw_line_chart(_sort_limit_monthly_df(monthly_summary, 12), f"{selected_plant} 월별 수율 (공정별)", "연-월")
            ...
            if selected_date_monthly == '전체':
                ...
                month_key = f"{dt_sel_m.year:04d}-{dt_sel_m.month:02d}"
        ...
        for proc in PROCESS_ORDER:
            ...
            proc_monthly_summary = _sort_limit_monthly_df(monthly_process_summaries.get(proc, pd.DataFrame(columns=[proc])), 12)
            proc_weekly_summary = _sort_limit_weekly_df(weekly_process_summaries.get(proc, pd.DataFrame(columns=[proc])), 12)
            _orig_ymin, _orig_ymax = y_min, y_max
            y_min, y_max = process_y_ranges.get(proc, (70.0, 100.0))
            with proc_cols[0]:
                draw_line_chart(proc_monthly_summary, f"{proc} 월간 수율", "연-월")
            with proc_cols[1]:
                draw_line_chart(_tail_weekly_df(proc_weekly_summary, 10), f"{proc} 주간 수율", "주차")
            y_min, y_max = _orig_ymin, _orig_ymax
        ...
        merged = load_and_merge_data(file_paths)
        df = _load_and_prepare_from_df(merged)
