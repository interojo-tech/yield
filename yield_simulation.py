...
def _year_week_sort_key(label: str) -> tuple[int, int, str]:
    m = re.search(r"(\d{4})-W(\d{1,2})", str(label))
    if m:
        return (int(m.group(1)), int(m.group(2)), str(label))
    m = re.search(r"W(\d{1,2})", str(label))
    if m:
...
def _sort_limit_weekly_df(summary: pd.DataFrame, limit: int | None = None) -> pd.DataFrame:
    if summary is None or summary.empty:
        return summary
    ordered_index = sorted(summary.index.tolist(), key=_year_week_sort_key)
    result = summary.loc[ordered_index]
...
def main() -> None:
    ...
    if st:
        ...
        df_week = df_week[df_week["week_num"].notna() & df_week["year_num"].notna()].copy()
        df_week["week_num"] = df_week["week_num"].astype(int)
        df_week["year_num"] = df_week["year_num"].astype(int)
        df_week["year_week"] = (
            df_week["year_num"].astype(str)
            + "-W"
            + df_week["week_num"].astype(str).str.zfill(2)
        )
        ...
        if not weekly_summary.empty:
            weekly_summary = _sort_limit_weekly_df(weekly_summary, None)
        ...
        try:
            ...
            try:
                monthly_summary.index = (
                    monthly_summary.index.get_level_values(0).astype(str)
        ...
        _prod_df["_week_key"] = (
            ...
            + "-W"
            + _prod_df["주차"].astype(str).str.extract(r"(\d+)")[0].fillna("").str.zfill(2)
        )
        ...
        if not product_weekly_pivot.empty:
            ...
            product_weekly_pivot = _sort_limit_weekly_df(product_weekly_pivot, 10)
        ...
        def compute_combined_summaries(full_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
            ...
            if not weekly_combined.empty:
                ...
                weekly_combined['주차'] = weekly_combined.apply(lambda row: f"{int(row['년도'])}-W{int(row['주차번호']):02d}", axis=1)
        ...
        if selected_period == "일별":
            ...
            pivot_source = _sort_limit_weekly_df(product_weekly_pivot, 10)
            index_label = "연-주"
            title_suffix = "주간"
        else:
