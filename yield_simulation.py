...
def main() -> None:
    ...
    if st:
        ...
        # Compute available week labels.  The '주차' column may contain values like
        # 'W3', '3', or other strings.  Extract the numeric part of each value,
        # discard weeks less than 2, sort ascending, and prefix with 'W'.  This
        # ensures the first week displayed is W2 and that W10, W11, etc. appear
        # after W9.  Any future weeks (e.g., W12, W13) will also append at the
        # end of the list.
        try:
            week_numbers_series = (
            ...
            week_numbers = sorted([w for w in week_numbers_series.unique().tolist() if w >= 2])
            weeks = [f"W{w}" for w in week_numbers]
        except Exception:
            weeks = []
        ...
        if selected_week != "전체":
            # Extract the numeric week number from the selected label (e.g., 'W2' -> 2)
            try:
                week_num = int(str(selected_week).lstrip('W'))
            except Exception:
                week_num = None
            if week_num is not None:
                # Convert the '주차' column to its numeric component for comparison
                week_col = (
                    filtered_df["주차"].astype(str).str.extract(r"(\d+)")[0].astype(float)
                )
                mask = week_col == float(week_num)
        ...
        # Create a composite key for year and week number.  Extract the numeric
        # week component from '주차' (e.g., 'W3' -> '3') so that sorting and
        # grouping work correctly regardless of the original format.
        df_week["year_week"] = (
        ...
        # Ensure weekly summary is sorted by numeric week number.  This places
        # W10/W11 after W9 and accommodates future weeks such as W12, W13, etc.
        if not weekly_summary.empty:
            try:
                # Extract the numeric part after the last '-' (week number).
                # Filter out week numbers less than 2 (exclude W1), sort by numeric
                # week, and assign new labels prefixed with 'W'.  This ensures
                # the first week displayed is W2 and that W10/W11 appear after W9.
                ...
                # Sort by week number
                week_nums_sorted = week_nums.sort_values()
                weekly_summary = weekly_summary.loc[week_nums_sorted.index]
                # Assign user-friendly labels like 'W2', 'W3', etc.
                new_labels = [f"W{n}" for n in week_nums_sorted.values]
