    ...
    if st:
        ...
        def compute_process_period_summary(
            ...
            Dict[str, pd.DataFrame]
                ...
                period label with one '수율' column in percent.
            """
            result: Dict[str, pd.DataFrame] = {}
            for proc in PROCESS_ORDER:
                proc_df = full_df[full_df["Process"] == proc].copy()
                if proc_df.empty:
                    result[proc] = pd.DataFrame(columns=["수율"])
                    continue
                if period == "monthly":
                    grouped = (
                ...
                summary = grouped.set_index("label")[["수율"]].round(2)
                result[proc] = summary
            return result
        ...
        def draw_combined_chart(combined_df: pd.DataFrame, title: str, xlabel: str, y_range: Tuple[float, float] = (80.0, 100.0)) -> None:
            ...
            if USE_PLOTLY:
                ...
                bars = ax_qty.bar(x, bar_vals, color='#86b3d1', label='양품량', alpha=0.5, zorder=1)
        ...
        if selected_plant == "전체 공장":
            st.markdown("---")
            st.subheader("전체 공장 공정별 월간/주간 수율")
            ...
            st.markdown("**공정별 월간 수율**")
            monthly_proc_cols = st.columns(2)
            for idx, proc in enumerate(PROCESS_ORDER):
                with monthly_proc_cols[idx % 2]:
                    proc_monthly_summary = monthly_process_summaries.get(proc, pd.DataFrame(columns=["수율"]))
                    draw_line_chart(proc_monthly_summary, f"{proc} 월간 수율", "연-월")

            st.markdown("**공정별 주간 수율**")
            weekly_proc_cols = st.columns(2)
            for idx, proc in enumerate(PROCESS_ORDER):
                with weekly_proc_cols[idx % 2]:
                    proc_weekly_summary = weekly_process_summaries.get(proc, pd.DataFrame(columns=["수율"]))
