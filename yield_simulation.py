...
def main() -> None:
    ...
    if st:
        ...
        _ = st.sidebar.selectbox("공정 선택 (불량)", options=defect_processes, index=0)
        _ = st.sidebar.selectbox("불량 유형 선택", options=defect_types, index=0)

        # ------------------------------------------------------------------
        # Header and title
        # ------------------------------------------------------------------
        # Inject custom CSS to style the radio button container and enlarge fonts.
        st.markdown(
            """
            <style>
            /* Style the radio group container */
            div.dashboard-radio > div {
                border: 2px solid #e0e0e0;
                padding: 12px;
                border-radius: 8px;
                margin-bottom: 12px;
            }
            /* Enlarge radio labels */
