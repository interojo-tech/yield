...
def main() -> None:
    ...
    if st:
        ...
        # Extract numeric week number from '주차' regardless of any 'W' prefix.  The
        # resulting _week_num column is numeric and allows grouping across
        # years without concatenating strings.  For example, 'W3' and '3'
        # both become 3.  If extraction fails, the week number becomes NaN
        # and those rows are excluded from the summary.
        df_week["_week_num"] = (
            df_week["주차"].astype(str).str.extract(r"(\d+)")[0].astype(float)
        )
        # Drop rows with missing week numbers to avoid grouping errors
        df_week = df_week[pd.notnull(df_week["_week_num"])]
        # Compute weekly summary grouped by year and numeric week number.  The
        # resulting DataFrame will have a MultiIndex (년도, _week_num).
        weekly_summary = summarise_and_percent(df_week, group_cols=("년도", "_week_num"))
        ...
        # Reorder weekly summary: extract numeric week numbers and sort ascending.
        # Map the existing index to labels of the form 'W#'.  This block
        # supports both simple and MultiIndex summaries.  Sorting ensures
        # that W2, W3, ..., appear in ascending order.  No weeks are
        # explicitly dropped.
        if not weekly_summary.empty:
            try:
                if isinstance(weekly_summary.index, pd.MultiIndex):
                    # When the index is a MultiIndex (년도, _week_num), use the
                    # second level as the week number.  Convert to int for
                    # sorting and label creation.
                    week_nums = weekly_summary.index.get_level_values(1).astype(int)
                else:
                    # Single-level index: attempt to extract digits from the label
                    week_nums = weekly_summary.index.to_series().astype(str).str.extract(r"(\d+)")[0].astype(int)
                # Determine sorted order by week number
                sorted_idx = week_nums.sort_values().index
                weekly_summary = weekly_summary.loc[sorted_idx]
                # Assign labels as 'W' + week number
                weekly_summary.index = [f"W{num}" for num in week_nums.loc[sorted_idx].values]
            except Exception:
                pass
        ...
        def draw_line_chart(summary: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                fig.update_yaxes(color='black', title_font=dict(color='black'))
        ...
        def draw_product_chart(pivot_df: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # axis title uses a black font.
                fig.update_yaxes(range=[50, 100], color='black', title_font=dict(color='black'))
                ...
                # Set x-axis label and ticks to black and specify title font colour
                fig.update_xaxes(title=xlabel, color='black', title_font=dict(color='black'))
                st.plotly_chart(fig, use_container_width=True)
            else:
        ...
        def draw_combined_chart(combined_df: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # Left y-axis for yield (%) with fixed range 60–90 and black labels/title
                fig.update_yaxes(title_text='수율 (%)', title_font=dict(color='black'), secondary_y=False, range=[60, 90], color='black')
                # Right y-axis for quantity (in millions) with black labels/title
                fig.update_yaxes(title_text='수량 (M)', title_font=dict(color='black'), secondary_y=True, color='black')
