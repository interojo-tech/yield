...
if 'plt' in globals():
    try:
        # Use DejaVu Sans as a safe default; Streamlit will respect these
        # settings when drawing charts.  Users can change this to another
        # font (e.g. NanumGothic or MalgunGothic) if those fonts are
        # available in their environment.
        plt.rcParams['font.family'] = 'DejaVu Sans'
        plt.rcParams['axes.unicode_minus'] = False
    except Exception:
        # If setting the font fails we do nothing so that matplotlib
...
def main() -> None:
    ...
    if st:
        ...
        # ------------------------------------------------------------------
        st.markdown(
            "<p style='font-size:14px; font-weight:bold; margin-bottom:0;'>인터로조 공정기술팀</p>",
        ...
        for idx, proc in enumerate(process_order):
            with kpi_cols[idx]:
                if proc in proc_summary.index:
                    ...
                    st.markdown(
                        f"<div style='background-color:#f7f9fc; border-radius:8px; padding:10px;'>"
        ...
        with kpi_cols[-1]:
            ...
            st.markdown(
                f"<div style='background-color:#e6f7ff; border-radius:8px; padding:10px;'>"
