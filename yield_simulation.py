...
def load_data(file_path: str | List[str]) -> pd.DataFrame:
    """Load one or more Excel files and return a combined production DataFrame.

    Parameters
    ----------
    file_path : str | List[str]
        Path to a single Excel file or a list of Excel files containing the
        sheet ``생산실적현황``.

    Returns
    -------
    pd.DataFrame
        A DataFrame with all rows from the matching Excel files combined.
    """
    if isinstance(file_path, (list, tuple)):
        frames: List[pd.DataFrame] = []
        for single_path in file_path:
            df_part = pd.read_excel(single_path, sheet_name="생산실적현황")
            df_part["source_file"] = os.path.basename(single_path)
    ...
    if "생산일자" in df.columns and not np.issubdtype(df["생산일자"].dtype, np.datetime64):
        df["생산일자"] = pd.to_datetime(df["생산일자"], errors="coerce")
    return df


def find_yield_files(base_dir: str) -> List[str]:
    """Find quarterly yield files first, then fall back to legacy filenames.

    Supported quarterly filename examples:
    - 공정기술팀 대시보드(수율)_25.1q.xlsx
    - 공정기술팀 대시보드(수율)_26.4q.xlsx

    If quarterly files are present, all matching quarter files are returned in
    chronological order so the existing dashboard can work without UI changes.
    """
    quarterly_patterns = [
        os.path.join(base_dir, "공정기술팀 대시보드(수율)_*.xlsx"),
        os.path.join(base_dir, "공정기술팀 대시보드_*.xlsx"),
    ]
    ...
    def _quarter_sort_key(file_name: str) -> tuple[int, int, str]:
        base_name = os.path.basename(file_name)
        match = re.search(r"(\d{2})\.(\d)q", base_name, re.IGNORECASE)
        if match:
            return (int(match.group(1)), int(match.group(2)), base_name)
        return (9999, 9999, base_name)

    quarter_files = sorted(set(quarter_files), key=_quarter_sort_key)
    quarter_files = [
        f for f in quarter_files
        if re.search(r"(\d{2})\.(\d)q", os.path.basename(f), re.IGNORECASE)
    ]
    if quarter_files:
...
def main() -> None:
    ...
    """
    # Determine the Excel file path(s). If quarterly files using the
    # YY.Qq naming pattern exist, automatically load and combine all of them
    # so the dashboard UI can remain unchanged.
    data_dir = os.path.dirname(__file__)
    file_path = find_yield_files(data_dir)
    if not file_path:
        print(
            "Error: No yield Excel files were found. Please place quarterly files "
            "such as '공정기술팀 대시보드(수율)_25.1q.xlsx' or "
