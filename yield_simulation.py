...
def main() -> None:
    ...
    if st:
        ...
        # Remove the yield Y‑axis range slider.  Instead, set a fixed
        # y‑axis range for all yield charts.  Yield percentages are
        # naturally bounded between 0 and 100, so use the full range
        # to ensure that low yields are visible.  If you wish to
        # customise this range in the future, adjust the values of
        # ``y_min`` and ``y_max`` here.
        # Fix the yield chart y‑axis range to 50–100%.  All yield graphs
        # throughout the dashboard will use this fixed range.  A lower
        # bound of 50% helps zoom into the typical operating range of
        # yields while still allowing room for minor variability.
        ...
        def draw_line_chart(summary: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # Apply the fixed yield y‑axis range (0–100).  The
                # ``y_min`` and ``y_max`` variables are defined above and
                ...
                try:
                    ...
                    pass
                # Set y-axis range based on the fixed values defined above
        ...
        def draw_product_chart(pivot_df: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # Set y-axis range to the fixed yield range defined above
                fig.update_yaxes(range=[y_min, y_max])
                ...
                try:
                    ...
                    pass
                # Align y-axis range with other charts using the fixed range
                ax.set_ylim(y_min, y_max)
                ax.tick_params(axis="x", rotation=45)
                ax.legend(bbox_to_anchor=(1.05, 1), loc="upper left")
                ax.grid(True, linestyle="--", alpha=0.3)
        ...
        def draw_combined_chart(combined_df: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # Left y-axis for yield (%) with the global fixed range
                fig.update_yaxes(title_text='수율 (%)', secondary_y=False, range=[y_min, y_max])
                ...
                # Use the fixed yield y‑axis range for the secondary axis
                ax_yield.set_ylim(y_min, y_max)
                # Set x-axis labels
        ...
        with tab_prod:
            ...
            if USE_PLOTLY:
                ...
                # Set the yield axis to the fixed range used throughout the dashboard
                fig.update_yaxes(title_text='수율 (%)', secondary_y=True, range=[y_min, y_max])
                st.plotly_chart(fig, use_container_width=True)
            else:
                ...
                ax2.set_ylabel('수율 (%)')
                # Set consistent y-axis range for yield using the fixed values
