...
def _year_week_sort_key(value: str) -> tuple[int, int, str]:
    """Sort helper for weekly labels such as '2026-W05' or 'W5'."""
    s = str(value)
...
def _tail_recent_weeks(df: pd.DataFrame, count: int = 10) -> pd.DataFrame:
    """Return the most recent weekly rows while preserving chronological order."""
    if df is None or df.empty:
        return df
    tmp = df.copy()
    ordered_index = sorted(tmp.index.tolist(), key=_year_week_sort_key)
    tmp = tmp.loc[ordered_index]
    return tmp.tail(min(count, len(tmp)))
...
def main() -> None:
    ...
    if st:
        ...
        # Reorder weekly pivot using year+week so the most recent 10 weeks can be selected safely.
        if not product_weekly_pivot.empty:
            sorted_week_index = sorted(product_weekly_pivot.index, key=_year_week_sort_key)
        ...
        with comb_cols[1]:
            st.markdown("**주간**")
            # 주간 그래프는 최근 10주만 표시합니다.
            draw_combined_chart(_tail_recent_weeks(weekly_combined, 10), "주간 양품량 및 수율", "주차", (65.0, 85.0))
        ...
        for proc in PROCESS_ORDER:
            ...
            proc_weekly_summary = _tail_recent_weeks(
                weekly_process_summaries.get(proc, pd.DataFrame(columns=[proc])),
                10,
            )
            _orig_ymin, _orig_ymax = y_min, y_max
        ...
        if selected_period == "일별":
            ...
            pivot_source = _tail_recent_weeks(product_weekly_pivot, 10)
            index_label = "연-주"
            title_suffix = "주간"
        else:
