...
def main() -> None:
    ...
    if st:
        ...
        with comb_cols[1]:
            st.markdown("**주간**")
            # Only this combined weekly chart is limited to the latest 10 weeks.
            # Prefer 2026 data when available, without changing any other weekly graphs.
            weekly_combined_plot = weekly_combined.copy()
            try:
                _weekly_year_labels = pd.Series(weekly_combined_plot.index.astype(str), index=weekly_combined_plot.index)
                _available_years = sorted({lbl.split('-W')[0] for lbl in _weekly_year_labels if '-W' in lbl and lbl.split('-W')[0].isdigit()})
                _target_year = '2026' if '2026' in _available_years else (_available_years[-1] if _available_years else None)
                if _target_year is not None:
                    weekly_combined_plot = weekly_combined_plot[_weekly_year_labels.str.startswith(f"{_target_year}-W")]
                if not weekly_combined_plot.empty:
                    weekly_combined_plot = weekly_combined_plot.tail(min(10, len(weekly_combined_plot)))
            except Exception:
