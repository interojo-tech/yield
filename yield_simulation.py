            ...
            if USE_PLOTLY:
                ...
                fig.add_trace(
                    go.Bar(
                        ...
                        name='양품량',
                        marker_color='#86b3d1',
                        # Format bar labels with 'M' suffix to indicate millions
                        text=[f"{v:.2f}M" for v in bar_values],
                ...
                for col, color in zip(['A관', 'C관', 'S관', '종합수율'], line_colors):
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            # Display the yield percentage as a label above each point
                            text=[f"{val:.2f}%" for val in line_values],
                ...
                # Fallback to matplotlib: line chart for yields (left y-axis) and bar chart for good quantity (right y-axis)
                fig, ax_yield = plt.subplots(figsize=(8, 4))
                x = np.arange(len(combined_df))
                # Plot yield lines on primary axis (left)
                line_colors = ['#e6194b', '#3cb44b', '#4363d8', '#f58231']
                for col, color in zip(['A관', 'C관', 'S관', '종합수율'], line_colors):
                    y_vals = (combined_df[col] * 100.0).values
                    ax_yield.plot(x, y_vals, marker='o', label=col, color=color)
                ...
                # Secondary axis for good quantity in millions
                ax_qty = ax_yield.twinx()
                bar_vals = (combined_df['양품량'] / 1_000_000.0).values
                bars = ax_qty.bar(x, bar_vals, color='#86b3d1', label='양품량', alpha=0.8)
                ax_qty.set_ylabel('수량 (M)')
                # Annotate bars with M labels
