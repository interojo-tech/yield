...
def main() -> None:
    ...
    if not file_paths:
        msg = (
            ...
            "같은 폴더에 아래 파일 중 하나가 있어야 합니다.\n"
            "- 공정기술팀 대시보드(수율)_25.1q.xlsx\n"
            "- 공정기술팀 대시보드(수율)_26.1q.xlsx\n"
            "- 공정기술팀 대시보드(수율).xlsx"
        )
        if st:
            try:
    ...
    if st:
        ...
        monthly_combined, _ = compute_combined_summaries(df)
        weekly_scope_df = df.copy()
        if selected_year != "전체":
            weekly_scope_df = weekly_scope_df[_safe_year_str(weekly_scope_df["년도"]) == selected_year].copy()
        _, weekly_combined = compute_combined_summaries(weekly_scope_df)
