...
def main() -> None:
    ...
    if st:
        ...
        # Sidebar selections
        # ------------------------------------------------------------------
        # 수율확인 섹션: 공장, 연도, 월, 주차, 공정 등의 선택을 제공합니다.  공장 선택은
        # 불량확인 섹션과 동일한 selectbox 방식으로 구현됩니다.
        st.sidebar.markdown("### 수율확인")
        # Include an "전체 공장" option so users can view all plants together.  Align naming
        # convention with the defect section for consistency.  The first option
        # becomes "전체 공장" followed by each plant name.  We avoid adding the
        # suffix "공장" to individual plant names so that filtering works
        # correctly (plant names already include 관/공장 info in the dataset).
        # Sidebar section for yield analysis.  Display a section header and
        # provide plant, year, month, week and process selectors.  Use the same
        # naming convention as the defect section ("전체 공장") so that the
        # options are consistent across sections.
        st.sidebar.markdown("### 수율확인")
