...
def find_latest_yield_file(base_dir: str) -> str | None:
    """가능한 수율 엑셀 파일 중 가장 최근 수정된 파일을 반환합니다."""
    patterns = [
        "공정기술팀 대시보드(수율).xlsx",
    ...
    search_roots = [base_dir, os.path.join(base_dir, "data"), os.getcwd()]
    candidates = []
    ...
    if not candidates:
        return None
    return sorted(set(candidates), key=lambda p: os.path.getmtime(p), reverse=True)[0]
...
def load_data(file_path: str) -> pd.DataFrame:
    ...
    sheet_name = "생산실적현황" if "생산실적현황" in excel_file.sheet_names else excel_file.sheet_names[0]
...
def main() -> None:
    ...
    file_path = find_latest_yield_file(os.path.dirname(__file__))
    if file_path is None:
        print(
            "Error: 수율 엑셀 파일을 찾을 수 없습니다. "
            "프로젝트 폴더 또는 data 폴더에 파일을 배치해 주세요."
        )
        return
    ...
    if st:
        ...
        def load_cached(path: str, file_mtime: float) -> pd.DataFrame:  # pragma: no cover
            _ = file_mtime
            return _load_and_prepare(path)

        file_mtime = os.path.getmtime(file_path)
        df = load_cached(file_path, file_mtime)
