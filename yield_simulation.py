...
def main() -> None:
    ...
    if st:
        ...
        # Prepare a DataFrame for yield summaries.  When a specific defect
        # classification is selected in the defect sidebar, filter the data
        # accordingly so that charts reflect only that defect type.  Otherwise,
        # use the full filtered dataset.
        df_for_summaries = filtered_df.copy()
        if selected_defect_type != "전체":
            # Filter rows where the classification matches the selected defect type.
            # The '신규분류요약' column contains defect classifications.
