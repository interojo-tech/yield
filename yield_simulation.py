...
def main() -> None:
    ...
    if st:
        ...
        if not product_weekly_pivot.empty:
            # Sort by year and week, then keep only the most recent 10 weeks.
            def _extract_year_week(x: str) -> tuple[int, int]:
                try:
                    s = str(x)
                    year_part, week_part = s.split('-W', 1)
                    return int(year_part), int(week_part)
                except Exception:
                    return (0, 0)
            sorted_week_index = sorted(product_weekly_pivot.index, key=_extract_year_week)
        ...
        # 종합 주간 그래프도 현재 선택된 조건(기본 2026년)을 기준으로 표시합니다.
        monthly_combined, weekly_combined = compute_combined_summaries(filtered_df)
        if not weekly_combined.empty:
            weekly_combined = weekly_combined.tail(min(10, len(weekly_combined)))
        ...
        if selected_period == "일별":
            ...
            pivot_source = product_weekly_pivot.tail(min(10, len(product_weekly_pivot))) if not product_weekly_pivot.empty else product_weekly_pivot
            index_label = "연-주"
            title_suffix = "주간"
        else:
