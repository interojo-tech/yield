...
def main() -> None:
    ...
    if st:
        ...
        # Determine available week options.  Extract numeric portions of
        # the '주차' column so we can build both individual week labels and
        # aggregated labels grouped by month.  The dataset may store weeks
        # as strings like 'W3' or just numbers.  We extract digits,
        # convert to integers and collect all unique week numbers.  This
        # ensures that week 1 (W1) and higher appear in ascending order.
        try:
            _week_nums_series = (
        ...
        # ------------------------------------------------------------------
        # Period selectors for weekly and daily views
        #
        # Build labels for weekly period selection.  Each label consists of
        # the month (e.g. '1월') and a list of week numbers within that
        # month (e.g. 'W2, W3, W4').  This provides a more natural UI for
        # selecting ranges such as '1월 - W2, W3, W4' and '2월 - W5, W6, W7, W8'.
        # The underlying mapping associates each label with the list of week
        # numbers that belong to that month.  When the user selects a start
        # and end label, all weeks from the start month through the end
        # month are included.  A '선택없음' option disables weekly filtering.
        try:
            df_weeks = df[['생산일자', '주차']].dropna().copy()
            df_weeks['week_num'] = df_weeks['주차'].astype(str).str.extract(r"(\d+)")[0].astype(int)
            df_weeks['month_num'] = df_weeks['생산일자'].dt.month
            # Group by month and collect week numbers
            month_week_groups = df_weeks.groupby('month_num')['week_num'].apply(lambda x: sorted(x.unique())).reset_index()
            week_labels_all = []
            week_label_to_nums = {}
            for _, row in month_week_groups.iterrows():
                month = int(row['month_num'])
                weeks_in_month = row['week_num']
                # Create label like '1월 - W2, W3, W4'
                week_parts = ', '.join([f"W{int(w)}" for w in weeks_in_month])
                label = f"{month}월 - {week_parts}"
                week_labels_all.append(label)
                week_label_to_nums[label] = weeks_in_month
        except Exception:
            week_labels_all = []
            week_label_to_nums = {}
        # Compose options including a '선택없음' entry.  Use a select_slider
        # to allow selecting a start and end label.  If either bound is
        # '선택없음', no weekly filter is applied.  Otherwise, all week
        # numbers in the inclusive range of months are combined.
        _weekly_slider_opts = ['선택없음'] + week_labels_all
        weekly_range_selected = st.sidebar.select_slider(
            "주간 기간 선택",
            options=_weekly_slider_opts,
            value=('선택없음', '선택없음'),
            help="주간 범위를 설정하면 선택된 첫 항목부터 두 번째 항목까지의 모든 주 데이터를 보여줍니다."
        )
        # Determine the list of week numbers to filter.  If either bound is
        # '선택없음', no weekly filter will be applied.
        if (
            isinstance(weekly_range_selected, tuple) or isinstance(weekly_range_selected, list)
        ) and (
            weekly_range_selected[0] != '선택없음' and weekly_range_selected[1] != '선택없음'
        ):
            try:
                start_label, end_label = weekly_range_selected
                start_idx = week_labels_all.index(start_label)
                end_idx = week_labels_all.index(end_label)
                # Ensure start_idx <= end_idx
                if start_idx > end_idx:
                    start_idx, end_idx = end_idx, start_idx
                # Collect all week numbers from start to end month inclusive
                selected_week_nums_list = []
                for i in range(start_idx, end_idx + 1):
                    label = week_labels_all[i]
                    if label in week_label_to_nums:
                        selected_week_nums_list.extend(week_label_to_nums[label])
                # Remove duplicates and sort
                _selected_week_nums = sorted(list(set(selected_week_nums_list)))
            except Exception:
                _selected_week_nums = None
        else:
            _selected_week_nums = None

        # Daily period selector.  Provide two modes: '선택없음' (no date filter)
        # and '기간 설정' (allow the user to choose a start and end date via a
        # date picker).  Place this below the weekly range selector.  When the
        # user selects a date range, parse it into (start, end); otherwise
        # default to the full available date range.
        daily_period_mode = st.sidebar.radio(
            "일별 기간 선택",
            options=['선택없음', '기간 설정'],
            index=0,
            help="일자 범위를 설정하면 선택된 기간 동안의 데이터만 표시됩니다."
        )
        if daily_period_mode == '기간 설정':
            _daily_value = (_min_dt, _max_dt) if _min_dt and _max_dt else None
            date_range_selected = st.sidebar.date_input(
                '일자 범위 선택',
                value=_daily_value,
                key='date_range_selected',
            )
            start_d, end_d = _parse_date_range(date_range_selected, _min_dt, _max_dt)
        else:
            start_d, end_d = _min_dt, _max_dt
        ...
        # These will be used to set defaults for the date picker when the user
        # chooses to apply a daily range filter.  Convert to Python date objects
        # for compatibility with st.date_input.  We compute these values
        # regardless of whether a date filter is applied so that a valid range
        # exists for parsing later.
        try:
            ...
            _min_dt = None
            _max_dt = None
        # Helper to parse a date input (single or range) into (start, end) dates.
        def _parse_date_range(selected, default_start, default_end):
            """Normalise a Streamlit date_input value into (start, end).

            Streamlit's date_input widget returns a single date, a tuple of
            two dates, or None.  This helper converts the input into a
            consistent (start, end) tuple.  If the user provides only one
            date, both start and end are set to that date.  If the input is
            empty or None, the provided defaults are returned.
            """
            if selected is None:
            ...
            # Use defaults if any bound is missing
            start = start or default_start
            end = end or default_end
            return start, end

        # We will capture the daily date range later after the user selection.
        # Initialize start_d and end_d to the full available range; these values
        # will be overridden if the user chooses to apply a daily filter.
        start_d, end_d = _min_dt, _max_dt
        # Initialize selected week numbers list.  When None, no weekly filter
        # will be applied.  The weekly filter will be built based on a
        # discrete slider defined below.
        _selected_week_nums = None
        ...
        # Fix the yield y-axis range to 70–100% for all charts.  This
        # provides a consistent view of yields across daily, weekly and
        # monthly charts while allowing more variation to be visible.  Removing
        # the slider prevents users from adjusting this range.
        ...
        # Use the defect‑filtered dataset for weekly summaries.  Do not filter
        # by date range here; instead, apply the user‑selected weekly range
        # filter via the _selected_week_nums list.  Extract the ISO week
        # number from the production date (생산일자) instead of parsing the
        # '주차' column, which may include year information (e.g., '2026-W2').
        # The ISO calendar week runs from 1–53.  Convert to integer for
        # grouping.  When _selected_week_nums is provided (i.e. both bounds
        # of the slider are actual labels), restrict the dataset to rows
        # whose week numbers fall within the selected range.
        df_week = df_for_summaries.copy()
        try:
            # Use dt.isocalendar().week to compute the ISO week number
            df_week["week_num"] = df_week["생산일자"].dt.isocalendar().week.astype(int)
        except Exception:
            # Fallback: extract digits after 'W' if isocalendar is unavailable
            df_week["week_num"] = df_week["주차"].astype(str).str.extract(r"W(\d+)")[0]
            df_week = df_week[df_week["week_num"].notna()].copy()
            df_week["week_num"] = df_week["week_num"].astype(int)
        # Apply the weekly range filter if the user has selected a range.
        if _selected_week_nums:
            try:
                df_week = df_week[df_week["week_num"].isin(_selected_week_nums)].copy()
            except Exception:
                pass
        ...
        # Weekly key: derive the ISO week number from the production date.  Use
        # dt.isocalendar().week to build keys like 'W1', 'W2', etc.  This
        # avoids issues when the '주차' column contains combined year-week
        # strings (e.g., '2026-W2') and ensures consistent week labels.
        try:
            iso_week_nums = _prod_df["생산일자"].dt.isocalendar().week.astype(int)
            _prod_df["_week_key"] = iso_week_nums.map(lambda w: f"W{w}")
        except Exception:
            # Fallback to using the original '주차' column if isocalendar
            # cannot be computed
            _prod_df["_week_key"] = (
        ...
        with tab_weekly:
            ...
            # Adjust the y‑axis range for weekly charts based on the selected plant.
            _orig_ymin, _orig_ymax = y_min, y_max
            if selected_plant == "전체 공장":
                y_min = 70.0
                y_max = 100.0
            ...
            # Restore global y-axis bounds for subsequent charts
            y_min, y_max = _orig_ymin, _orig_ymax
        ...
        with tab_monthly:
            ...
            # Draw the monthly summary chart before the search and table.  Adjust
            # the y-axis range based on the selected plant.  For the entire
            # plant, show a wider range (70–100%) to reveal more variation.
            _orig_ymin, _orig_ymax = y_min, y_max
            if selected_plant == "전체 공장":
                y_min = 70.0
                y_max = 100.0
