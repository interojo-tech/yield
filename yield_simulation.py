...
def main() -> None:
    ...
    if st:
        ...
        if "주차" in df.columns:
            try:
                # Extract numeric portions from the week column
                num_series = (
                    df["주차"].astype(str).str.extract(r"(\d+)", expand=False)[0]
                )
                # Convert to integers, ignoring missing values
                week_nums = num_series.dropna().astype(int)
                unique_week_nums = sorted(set(week_nums))
        ...
        def draw_line_chart(summary: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                fig.update_layout(
                    legend_title_text="공정", xaxis_title=xlabel, yaxis_title="수율 (%)"
                )
