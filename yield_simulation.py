...
def main() -> None:
    ...
    if st:
        ...
        if selected_year != "전체":
            _date_base_df = _date_base_df[_date_base_df["년도"].astype(int).astype(str) == selected_year]
        ...
        # 월간은 연도-월 순서로 안전하게 정렬한 뒤 최근 12개월만 표시합니다.
        monthly_combined_plot = monthly_combined.copy()
        if not monthly_combined_plot.empty:
            monthly_index_series = pd.Series(monthly_combined_plot.index.astype(str), index=monthly_combined_plot.index)
            monthly_parts = monthly_index_series.str.extract(r"(?P<year>\d{4})-(?P<month>\d{1,2})")
            monthly_sort_key = pd.to_datetime(
                monthly_parts["year"].fillna("1900") + "-" + monthly_parts["month"].fillna("1") + "-01",
                errors="coerce",
            )
            monthly_combined_plot = monthly_combined_plot.assign(_sort_key=monthly_sort_key)
            monthly_combined_plot = monthly_combined_plot.sort_values(by="_sort_key")
            monthly_combined_plot = monthly_combined_plot.drop(columns=["_sort_key"])
            if len(monthly_combined_plot) > 12:
                monthly_combined_plot = monthly_combined_plot.tail(12)

        # 주간은 compute_combined_summaries()에서 이미 연도+주차 기준으로 정렬되어 있으므로
        # 현재 순서를 유지한 채 최근 12주만 표시합니다.
        weekly_combined_plot = weekly_combined.copy()
        if not weekly_combined_plot.empty and len(weekly_combined_plot) > 12:
            weekly_combined_plot = weekly_combined_plot.tail(12)
        ...
        with tab_daily:
            ...
            selected_date_daily = st.selectbox(
                '생산일자 검색',
                options=(['검색안함'] + unique_dates) if selected_plant == "전체 공장" else (['전체'] + unique_dates),
                index=0,
                key='date_search_daily'
            )
            if selected_plant == "전체 공장" and selected_date_daily == '검색안함':
                summary_daily_filtered = None
            elif selected_date_daily == '전체':
            ...
            # For 전체 공장에서 '검색안함'을 선택한 경우 summary_daily_filtered는 None이며
            # 이때는 표를 숨긴다.
        ...
        with tab_weekly:
            ...
            if selected_plant == "전체 공장" and selected_date_weekly == '검색안함':
                summary_weekly_filtered = None
            elif selected_date_weekly == '전체':
            ...
            # a table should actually be shown. For 전체 공장에서 '검색안함'을 선택한
        ...
        with tab_monthly:
            ...
            if selected_plant == "전체 공장" and selected_date_monthly == '검색안함':
                summary_monthly_filtered = None
            elif selected_date_monthly == '전체':
        ...
        if selected_plant == "전체 공장":
            st.markdown("---")
            st.subheader("공정별 수율")
            monthly_process_summaries = compute_process_period_summary(df, "monthly")
            weekly_process_summaries = compute_process_period_summary(df, "weekly")
            process_y_ranges = {
                "사출조립": (85.0, 100.0),
                "분리": (80.0, 95.0),
