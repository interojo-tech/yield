    ...
    if st:
        ...
        def draw_combined_chart(combined_df: pd.DataFrame, title: str, xlabel: str) -> None:
            ...
            if USE_PLOTLY:
                ...
                # Lines for yields
                for col, color in zip(['A관', 'C관', 'S관', '종합수율'], ['#5470c6', '#91cc75', '#fac858', '#ee6666']):
                    fig.add_trace(
                        go.Scatter(
                            x=combined_df.index,
                            y=combined_df[col] * 100.0,  # convert to percent
                            name=col,
                            mode='lines+markers',
                            marker=dict(size=6),
                            line=dict(color=color),
                            text=(combined_df[col] * 100.0).round(2),
                ...
                # Fallback to matplotlib: bar chart for good quantity and multiple lines on secondary y-axis
                fig, ax1 = plt.subplots(figsize=(8, 4))
                x = np.arange(len(combined_df))
                # Bar for good quantity
                ax1.bar(x, combined_df['양품량'], color='#86b3d1', label='양품량')
                ax1.set_xlabel(xlabel)
                ax1.set_ylabel('수량')
                # Secondary axis for yields
                ax2 = ax1.twinx()
                for col, color in zip(['A관', 'C관', 'S관', '종합수율'], ['#5470c6', '#91cc75', '#fac858', '#ee6666']):
                    ax2.plot(x, combined_df[col] * 100.0, marker='o', label=col, color=color)
                ax2.set_ylabel('수율 (%)')
                ax2.set_ylim(50, 100)
                # Set x-axis ticks to index labels
                ax1.set_xticks(x)
