...
def main() -> None:
    ...
    def _load_and_prepare(path: str) -> pd.DataFrame:
        ...
        # Remove any records for the '몰드인쇄' process.  This ensures that the
        # sidebar process selection and all yield/production charts exclude
        # this process.  No other modifications are applied to the data.
        if "Process" in df_local.columns:
            df_local = df_local[df_local["Process"] != "몰드인쇄"].copy()
    ...
    if st:
        ...
        # Fix the yield y-axis range (50–100%).  Removing the slider eliminates
        # the range control while preserving the default range.
