...
def main() -> None:
    ...
    def _load_and_prepare(path: str) -> pd.DataFrame:
        ...
        # Sort by production date so that newly added rows appear in chronological order.
        df_local = df_local.sort_values(by="생산일자").reset_index(drop=True)
        return df_local
    ...
    if st:
        ...
        # Include the file's modification time in the cache key so that updates
        # to the Excel file invalidate the cached data.  The mtime argument
        # is not used inside the function body but is part of the cache
        # signature.  This also ensures that caches from prior versions
        # (with a different signature) are not reused.
        @st.cache_data  # type: ignore[misc]
        def load_cached(path: str, mtime: float) -> pd.DataFrame:  # pragma: no cover
            return _load_and_prepare(path)

        df = load_cached(file_path, os.path.getmtime(file_path))
