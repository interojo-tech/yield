    ...
    if st:
        ...
        def compute_combined_summaries(full_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
            ...
            if not weekly_combined.empty:
                # 종합 수율 및 양품량의 주간 그래프는 2026년도 데이터에서 최근 10주만 표시합니다.
                weekly_combined = weekly_combined[weekly_combined['년도'].astype(str) == '2026'].copy()
                weekly_combined = weekly_combined.sort_values(['년도', '주차번호']).tail(10)
                # Create string index like '2026-W2', '2026-W3', etc.
                weekly_combined['주차'] = weekly_combined.apply(
                    lambda row: f"{int(row['년도'])}-W{int(row['주차번호'])}", axis=1
                )
