...
def main() -> None:
    ...
    def _discover_yield_files(base_dir: str) -> list[str]:
        """Find quarterly yield files first, then fall back to legacy filenames."""
        discovered: list[str] = []
        try:
        ...
        for name in entries:
            if not name.lower().endswith('.xlsx'):
                continue
            if name.startswith('공정기술팀 대시보드(수율)_'):
                discovered.append(os.path.join(base_dir, name))
    ...
    file_paths = _discover_yield_files(script_dir)
    if not file_paths:
        print(
            "Error: No yield Excel files were found. Please place quarterly files like "
            "'공정기술팀 대시보드(수율)_26.1q.xlsx' in the same directory as this script."
        )
        return
    ...
    # interaction.
    def _load_and_prepare(paths: list[str]) -> pd.DataFrame:
        frames: list[pd.DataFrame] = []
        for path in paths:
            df_single = load_data(path)
            df_single['source_file'] = os.path.basename(path)
            frames.append(df_single)
        df_local = pd.concat(frames, ignore_index=True) if frames else pd.DataFrame()
    ...
    if st:
        ...
        def load_cached(paths: tuple[str, ...]) -> pd.DataFrame:  # pragma: no cover
            return _load_and_prepare(list(paths))
        ...
        # ------------------------------------------------------------------
        # Basic temporal selections: year, month and week
        # ------------------------------------------------------------------
        # We gather year, month and week selections first.  The process selection
        # will appear after the period range selectors so that users can
        # specify start/end periods before filtering by process.  This ordering
        # matches the desired UI layout where the weekly and daily period
        # filters sit directly below the week selector.
        ...
        # ------------------------------------------------------------------
        # Compute available weeks and dates for the period selectors
        #
        # Build a temporary DataFrame filtered by plant, year, month, week and
        # process selections (excluding process here since it is selected
        # later).  This temporary subset is used to derive the set of weeks
        # and the min/max dates so that the period selectors reflect only
        # available data.  By computing this early we can present the
        # period selectors immediately below the week selector.
        try:
            tmp_df = df.copy()
            if selected_plant != "전체 공장":
                tmp_df = tmp_df[tmp_df["공장"] == selected_plant]
            if selected_year != "전체":
                tmp_df = tmp_df[tmp_df["년도"].astype(int).astype(str) == selected_year]
            if selected_month != "전체":
                tmp_df = tmp_df[tmp_df["월"].astype(str) == selected_month]
            if selected_week != "전체":
                # Extract numeric week number from the selected week label and filter
                try:
                    _wk_num_tmp = int(str(selected_week).lstrip("W"))
                except Exception:
                    _wk_num_tmp = None
                if _wk_num_tmp is not None:
                    _wk_col_num_tmp = (
                        tmp_df["주차"].astype(str).str.extract(r"(\d+)")[0].astype(float)
                    )
                    tmp_df = tmp_df[_wk_col_num_tmp == float(_wk_num_tmp)]
            # Determine min and max production dates for the daily range picker.
            if not tmp_df.empty and "생산일자" in tmp_df.columns:
                _tmp_min_dt = tmp_df["생산일자"].dropna().min()
                _tmp_max_dt = tmp_df["생산일자"].dropna().max()
                _tmp_min_dt = _tmp_min_dt.date() if not pd.isna(_tmp_min_dt) else None
                _tmp_max_dt = _tmp_max_dt.date() if not pd.isna(_tmp_max_dt) else None
            else:
                _tmp_min_dt, _tmp_max_dt = None, None
        except Exception:
            _tmp_min_dt, _tmp_max_dt = None, None
            tmp_df = pd.DataFrame()

        # ------------------------------------------------------------------
        # Build week labels for the weekly range selector.  Each label combines
        # the month (e.g. '1월') with the week number (e.g. 'W2') so that users
        # understand which calendar month a particular week belongs to.  These
        # labels serve as options for the start and end week selectors below.
        week_entries = []
        try:
            if not tmp_df.empty and "주차" in tmp_df.columns:
                _tmp_week_nums_series = (
                    tmp_df["주차"]
                    .dropna()
                    .astype(str)
                    .str.extract(r"(\d+)")[0]
                ...
                unique_weeks_sorted = sorted(_tmp_week_nums_series.unique().tolist())
                for w in unique_weeks_sorted:
                    # Determine the representative month for this week.  Use the most
                    # frequent month value in the filtered dataset for that week.
                    try:
                        # Filter rows corresponding to this week number
                        _wk_mask = (
                            tmp_df["주차"]
                            .astype(str)
                            .str.extract(r"(\d+)")[0]
                        ...
                        row = tmp_df[_wk_mask]
                        if not row["월"].dropna().empty:
                            month_value = (
                                row["월"].dropna().astype(str).mode().iloc[0]
                            )
                        else:
                            month_value = ""
                    except Exception:
                        month_value = ""
                    if month_value:
                        label = f"{month_value}-W{w}"
                    else:
                        label = f"W{w}"
                    week_entries.append(label)
        except Exception:
            week_entries = []

        # Options for the start and end week selectors.  Include '선택없음' as the
        # first entry so that users can choose not to apply a weekly range.
        week_options_range = ["선택없음"] + week_entries
        # Determine default indices: start defaults to '선택없음', end defaults to the
        # last available week if any exist; otherwise '선택없음'.
        if week_entries:
            default_start_idx = 0
            default_end_idx = len(week_entries)
        else:
            default_start_idx = 0
            default_end_idx = 0
        # Present two selectboxes for weekly start and end.  These will replace
        # the previous select_slider and allow users to specify a range without
        # using a scroll slider.  Placing them here ensures they appear
        # immediately below the week selector.
        selected_week_start = st.sidebar.selectbox(
            "주간 기간 시작", options=week_options_range, index=default_start_idx
        )
        selected_week_end = st.sidebar.selectbox(
            "주간 기간 종료", options=week_options_range, index=default_end_idx
        )

        # Daily date range picker with clear (X) button.
        # Use None as the initial value so that selecting '선택없음' (no selection) is supported.
        # Bind the selectable dates using the minimum and maximum dates from tmp_df.  If
        # these are unavailable, no bounds are provided.  To allow clearing the
        # selection after use, we place the date input and a clear button in adjacent
        # columns.  Clicking the clear button resets the date picker value to None.
        date_cols = st.sidebar.columns([4, 1])
        with date_cols[0]:
            if _tmp_min_dt and _tmp_max_dt:
                date_range_daily = st.date_input(
                    "일별 기간 선택",
                    value=st.session_state.get("date_range_daily", None),
                    min_value=_tmp_min_dt,
                    max_value=_tmp_max_dt,
        ...
        with date_cols[1]:
            # Provide an 'X' button to clear the date selection.  When clicked,
            # reset the session state key associated with the date picker.
            if st.button("❌", key="clear_date_range_daily"):
                st.session_state["date_range_daily"] = None

        # Immediately after rendering the date input, inject JavaScript to hide
        # the default 'Choose a date range' message and rename 'None' to
        # '선택없음' within the date picker.  This script polls the parent
        # document for these strings and modifies them when found.  Using a
        # zero-height HTML component ensures no additional space is consumed.
        st.components.v1.html(
            """
            <script>
            // Poll until the date picker overlay elements are present, then
            // remove the default placeholder text and rename the 'None' button.
            const intervalId = setInterval(() => {
                const spans = window.parent.document.querySelectorAll('span');
                let found = false;
                spans.forEach(el => {
                    if (el.innerText === 'Choose a date range') {
                        el.innerText = '';
                        found = true;
                    }
                ...
                // Stop polling once modifications are made to avoid unnecessary work
                if (found) {
        ...
        # After the period selectors, provide the process selection.  Placing the
        # process selector here means it appears below the weekly and daily
        # filters, matching the desired hierarchy.  Users can now choose a
        # specific process after defining their temporal range.
        selected_process = st.sidebar.selectbox(
            "공정 선택", options=process_options, index=0
        )

        # Previously the date and week range selectors were defined here and
        # computed their own temporary filtered dataset (tmp_df).  These
        # selectors have been moved above the process selection to improve
        # layout.  As a result, this block is no longer needed and has been
        # removed.  The variables `_tmp_min_dt`, `_tmp_max_dt` and `tmp_df`
        # are now computed earlier in the sidebar setup.
        ...
        #
        # Compute the earliest and latest production dates in the filtered dataset.
        # These values are derived from df_for_summaries (after defect filtering)
        # and are used as fallbacks if the user does not select specific
        # start/end dates in the daily picker.  The weekly date range now
        # derives from the custom weekly range slider defined earlier.
        try:
            _min_dt = df_for_summaries['생산일자'].dropna().min().date()
            _max_dt = df_for_summaries['생산일자'].dropna().max().date()
        except Exception:
            _min_dt = None
            _max_dt = None

        # Reuse the _parse_date_range helper to normalise daily date inputs.
        ...
        # Normalise the daily date input into start and end dates.  If the user
        # does not select any date, fall back to the full range from _min_dt to _max_dt.
        start_d, end_d = _parse_date_range(date_range_daily, _min_dt, _max_dt)

        # For weekly summaries, use the full date range from df_for_summaries.  The
        # week range filtering will be applied later based on the start and end
        # week selectors (selected_week_start and selected_week_end).
        start_w, end_w = _min_dt, _max_dt
        ...
        if not weekly_summary.empty:
            try:
                ...
                pass
        # Apply the selected week range filter to the weekly summary.  The
        # start and end labels come from the two selectboxes defined above
        # (selected_week_start and selected_week_end).  Extract numeric
        # week numbers from the labels and retain only rows whose week
        # numbers fall within the selected range.  When both selectors
        # are '선택없음' no filtering is applied.
        try:
            start_label = selected_week_start
            end_label = selected_week_end
            import re as _re
            def _extract_week_num(label):
                _m = _re.search(r"W(\d+)", str(label))
                return int(_m.group(1)) if _m else None
            sw = _extract_week_num(start_label)
            ew = _extract_week_num(end_label)
            # If both selections are defined and sw > ew, swap to ensure correct order
            if sw is not None and ew is not None and sw > ew:
                sw, ew = ew, sw
            if sw is not None or ew is not None:
                # Build a Series of numeric week numbers from the weekly summary index
                _idx_week_nums = weekly_summary.index.to_series().astype(str).str.extract(r"(\d+)")[0].astype(int)
                if sw is not None:
                    weekly_summary = weekly_summary[_idx_week_nums >= sw]
                if ew is not None:
                    weekly_summary = weekly_summary[_idx_week_nums <= ew]
        except Exception:
            pass
            ...
            if USE_PLOTLY:
                ...
                for i, trace in enumerate(fig.data):
                    ...
                    if isinstance(name, str) and name.strip() == "종합수율":
                        ...
                        fig.data[i].update(line=dict(width=4), marker=dict(size=8))
                        ...
                        fig.data[i].update(line=dict(width=2), marker=dict(size=6))
        ...
        def compute_combined_summaries(full_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
            ...
            for (year, month), group in full_df.groupby(['년도', '월']):
                # Total good quantity across all plants for the 누수/규격검사 process only.
                # If the process is absent in the group, sum returns 0.  This
                # aligns the monthly good quantity with the leakage/standard
                # inspection process as requested.
                try:
            ...
            for (year, week_num), group in full_df.groupby(['년도', '_week_num']):
                # Include all week numbers (including week 1)
                # Total good quantity across all plants for the 누수/규격검사 process only.
                try:
                    total_good = group[group['Process'] == '누수/규격검사']['양품수량'].sum()
                except Exception:
        ...
        def draw_combined_chart(combined_df: pd.DataFrame, title: str, xlabel: str, y_range: Tuple[float, float] = (80.0, 100.0)) -> None:
            ...
            if USE_PLOTLY:
                ...
                bars = ax_qty.bar(x, bar_vals, color='#86b3d1', label='양품량', alpha=0.3, zorder=1)
                ...
                for col, color in zip(['A관', 'C관', 'S관', '종합수율'], darkened):
                    ...
                    ax_yield.plot(x, y_vals, marker='o', markersize=6, linewidth=3.0, label=col, color=color, zorder=3)
        ...
        with comb_cols[0]:
            ...
            draw_combined_chart(monthly_combined, "월간 양품량 및 수율", "연-월", (65.0, 80.0))
        with comb_cols[1]:
            st.markdown("**주간**")
        ...
        with tab_daily:
            ...
            # Draw the daily summary chart.  By default, show only the most recent
            # 30 days to keep the chart focused on recent performance.  However,
            # if the user selected a specific date range via the date picker,
            # display the full range of available dates within that selection.
            if not daily_summary.empty:
                try:
                    ds = daily_summary.copy()
                    ds.index = pd.to_datetime(ds.index)
                    ds = ds.sort_index()
                    # Determine whether a date range has been selected.  When
                    # date_range_daily is not None (and not empty), show all
                    # available dates; otherwise display only the last 30 days.
                    _show_all = False
                    try:
                        # date_range_daily may be None, a date, or a tuple/list
                        _val = date_range_daily
                ...
                daily_summary_plot = daily_summary
            # Adjust the y‑axis range for daily charts.  When the entire plant
            # ("전체 공장") is selected, use a wider range (70–100) to reveal
            # more variation.  For individual plants, retain the default
            # high‑range (80–100).  Temporarily override the global y_min
            # and y_max variables used by draw_line_chart; restore them
            # immediately after the chart is drawn.
        ...
        with tab_weekly:
            ...
            # Draw the weekly summary chart.  By default, display only the
            # most recent 10 weeks to focus on recent performance.  If the
            # user selected a specific week range using the start/end
            # selectors, display the entire range of weeks within that
            # selection (i.e., do not truncate to 10).  When no range is
            # selected (both selectors show '선택없음'), fall back to the
            # last 10 weeks.
            if not weekly_summary.empty:
                # Determine whether a week range has been selected
                _week_range_selected = False
                try:
                    if selected_week_start != '선택없음' or selected_week_end != '선택없음':
                        _week_range_selected = True
                except Exception:
                    _week_range_selected = False
                if _week_range_selected:
                    weekly_summary_plot = weekly_summary
                else:
                    weekly_summary_plot = weekly_summary.tail(min(10, len(weekly_summary)))
            else:
                weekly_summary_plot = weekly_summary
            # Adjust the y-axis range for weekly charts.  When the entire plant
            # is selected, use a wider range (70–100); otherwise use the
            # default high-range (80–100).  Temporarily override y_min/y_max
            # and restore them after drawing the chart.
            _orig_ymin, _orig_ymax = y_min, y_max
            if selected_plant == "전체 공장":
                y_min = 70.0
                y_max = 100.0
        ...
        with tab_monthly:
            ...
            # Adjust the y-axis range for monthly charts.  When the entire plant
            # is selected, use a wider range (70–100); otherwise use the
            # default high-range (80–100).  Temporarily override y_min/y_max
            # and restore them after drawing the chart.
            _orig_ymin, _orig_ymax = y_min, y_max
