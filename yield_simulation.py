...
def main() -> None:
    ...
    # List of possible Excel filenames for yield data.  The expected
    # filename is '공정기술팀 대시보드(수율)' with an .xlsx extension.
    # A variant with a space before the parenthesis is also tried for
    # compatibility.  Other historical filenames have been removed per
    ...
    if file_path is None:
        print(
            "Error: None of the expected yield Excel files were found. "
            "Please place '공정기술팀 대시보드(수율).xlsx' in the same directory "
            "as this script."
        )
        return
    ...
    if st:
        ...
        # 데이터 캐싱을 사용하여 엑셀 파일이 변경될 때마다 캐시가 무효화되도록 합니다.
        # mtime을 두 번째 인자로 넘겨 캐시 키에 포함시킴으로써 파일이 업데이트되면 자동으로 새로 로드됩니다.
        @st.cache_data(show_spinner=False)
        def _get_prepared_df(path: str, mtime: float) -> pd.DataFrame:
            return _load_and_prepare(path)

        df = _get_prepared_df(file_path, os.path.getmtime(file_path))
